...
type diskReader struct{ db siladb.KeyValueStore }
...
type generator struct {
	...
	db    siladb.KeyValueStore // Key-value store containing the snapshot data
...
func newGenerator(db siladb.KeyValueStore, noBuild bool, progress []byte, stats *generatorStats) *generator {
	if stats == nil {
...
func journalProgress(db siladb.KeyValueWriter, marker []byte, stats *generatorStats) {
...
func (g *generator) checkAndFlush(ctx *generatorContext, current []byte) error {
	...
	if ctx.batch.ValueSize() > siladb.IdealBatchSize || abort != nil {
