...
func readStateHistoryMeta(reader siladb.AncientReader, id uint64) (*meta, error) {
...
func readStateHistory(reader siladb.AncientReader, id uint64) (*stateHistory, error) {
...
func readStateHistories(freezer siladb.AncientReader, start uint64, count uint64) ([]history, error) {
...
func writeStateHistory(writer siladb.AncientWriter, dl *diffLayer) error {
...
func checkStateHistories(reader siladb.AncientReader, start, count uint64, check func(*meta) error) error {
	for count > 0 {
		number := count
		if number > 10000 {
