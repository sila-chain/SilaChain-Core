...
func truncateFromHead(store siladb.AncientStore, typ historyType, nhead uint64) (int, error) {
...
// on the given parameters. It returns the number of items that were removed.
func truncateFromTail(store siladb.AncientStore, typ historyType, ntail uint64) (int, error) {
	ohead, err := store.Ancients()
	if err != nil {
		return 0, err
	}
...
func purgeHistory(store siladb.ResettableAncientStore, disk siladb.KeyValueStore, typ historyType) {
	if store == nil {
		return
	}
...
func syncHistory(stores ...siladb.AncientWriter) error {
...
// unexpected panics.
func repairHistory(db siladb.Database, isUBT bool, readOnly bool, stateID uint64, enableTrienode bool) (siladb.ResettableAncientStore, siladb.ResettableAncientStore, error) {
	...
	var trienodes siladb.ResettableAncientStore
	if enableTrienode {
	...
	truncate := func(store siladb.AncientStore, typ historyType, nhead uint64) {
		if store == nil {
			return
		}
