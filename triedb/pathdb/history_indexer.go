...
func loadIndexMetadata(db siladb.KeyValueReader, typ historyType) *indexMetadata {
	var blob []byte
	switch typ {
	case typeStateHistory:
...
func storeIndexMetadata(db siladb.KeyValueWriter, typ historyType, last uint64) {
	m := indexMetadata{
...
func deleteIndexMetadata(db siladb.KeyValueWriter, typ historyType) {
	switch typ {
	case typeStateHistory:
...
type batchIndexer struct {
	...
	db      siladb.KeyValueStore       // Key-value database used to store or delete index data.
}

// newBatchIndexer constructs the batch indexer with the supplied mode.
func newBatchIndexer(db siladb.KeyValueStore, delete bool, typ historyType) *batchIndexer {
	return &batchIndexer{
...
func (b *batchIndexer) finish(force bool) error {
	...
	var (
		...
		batch     = b.db.NewBatchWithSize(siladb.IdealBatchSize)
		batchSize int
		batchMu   sync.RWMutex

		writeBatch = func(fn func(batch siladb.Batch)) error {
			batchMu.Lock()
	...
	for ident, list := range b.index {
		...
		eg.Go(func() error {
			if !b.delete {
				...
				return writeBatch(func(batch siladb.Batch) {
					iw.finish(batch)
				})
			} else {
...
func indexSingle(historyID uint64, db siladb.KeyValueStore, freezer siladb.AncientReader, typ historyType) error {
	start := time.Now()
	defer func() {
...
func unindexSingle(historyID uint64, db siladb.KeyValueStore, freezer siladb.AncientReader, typ historyType) error {
	start := time.Now()
	defer func() {
		if typ == typeStateHistory {
...
type indexIniter struct {
	state     *initerState
	disk      siladb.Database
	freezer   siladb.AncientStore
...
func newIndexIniter(disk siladb.Database, freezer siladb.AncientStore, typ historyType, lastID uint64, noWait bool) *indexIniter {
	initer := &indexIniter{
...
type historyIndexer struct {
	...
	pruner  *indexPruner
	typ     historyType
	disk    siladb.KeyValueStore
	freezer siladb.AncientStore
}

// checkVersion checks whether the index data in the database matches the version.
func checkVersion(disk siladb.KeyValueStore, typ historyType) {
	var blob []byte
	if typ == typeStateHistory {
...
func newHistoryIndexer(disk siladb.Database, freezer siladb.AncientStore, lastHistoryID uint64, typ historyType, noWait bool) *historyIndexer {
