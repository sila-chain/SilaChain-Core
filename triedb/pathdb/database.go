...
type Database struct {
	...
	stateFreezer siladb.ResettableAncientStore // Freezer for storing state histories, nil possible in tests
	stateIndexer *historyIndexer              // History indexer historical state data, nil possible

	trienodeFreezer siladb.ResettableAncientStore // Freezer for storing trienode histories, nil possible in tests
...
func New(diskdb siladb.Database, config *Config, isUBT bool) *Database {
	if config == nil {
		config = Defaults
	}
