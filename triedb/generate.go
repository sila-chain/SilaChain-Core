...
func openRangeIterators(db siladb.Database, start common.Hash) *rangeIterators {
...
func (r *rangeIterators) flushIfFull(batch siladb.Batch, where string) error {
...
func openFlatIterator(db siladb.Database, prefix, start []byte, suffixLen int) *internal.HoldableIterator {
...
func reopenFlatIterator(db siladb.Database, old *internal.HoldableIterator, prefix []byte, suffixLen int) *internal.HoldableIterator {
	if !old.Next() {
		old.Release()
...
func generatePartition(ctx context.Context, cancel <-chan struct{}, db siladb.Database, scheme string, partition byte, rangeStart, rangeEnd common.Hash, c *genCounters) ([]byte, error) {
	iters := openRangeIterators(db, rangeStart)
	defer iters.release()

	batch := db.NewBatchWithSize(siladb.IdealBatchSize)
...
func GenerateTrie(db siladb.Database, scheme string, root common.Hash, cancel <-chan struct{}) (GenerateStats, error) {
...
func GenerateTrieWithProgress(db siladb.Database, scheme string, root common.Hash, cancel <-chan struct{}, prog *atomic.Uint64) (GenerateStats, error) {
	var (
		start        = time.Now()
		c            genCounters
...
func assembleRoot(db siladb.Database, scheme string, partitionBlobs [numPartitions][]byte) (common.Hash, error) {
	var (
		populated int
