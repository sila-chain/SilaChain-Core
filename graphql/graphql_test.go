...
func newGQLService(t *testing.T, stack *node.Node, shanghai bool, gspec *core.Genesis, genBlocks int, genfunc func(i int, gen *core.BlockGen)) (*handler, []*types.Block) {
	...
	var silaEngine = beacon.New(silaash.NewFaker())
	if shanghai {
	...
	silaBackend, err := sila.New(stack, silaConf)
	if err != nil {
		t.Fatalf("could not create sila backend: %v", err)
	}
	// Create some blocks and import them
	chain, _ := core.GenerateChain(params.AllSilaashProtocolChanges, silaBackend.BlockChain().Genesis(),
		silaEngine, silaBackend.ChainDb(), genBlocks, genfunc)
	_, err = silaBackend.BlockChain().InsertChain(chain)
	if err != nil {
		t.Fatalf("could not create import blocks: %v", err)
	}
	// Set up handler
	filterSystem := filters.NewFilterSystem(silaBackend.APIBackend, filters.Config{})
	handler, err := newHandler(stack, silaBackend.APIBackend, filterSystem, []string{}, []string{})
	if err != nil {
		t.Fatalf("could not create graphql service: %v", err)
	}
