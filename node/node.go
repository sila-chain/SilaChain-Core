...
func New(conf *Config) (*Node, error) {
	...
	node.httpAuth.disableHTTP2 = true // SilaEngine API does not need HTTP/2
...
func (n *Node) OpenDatabaseWithOptions(name string, opt DatabaseOptions) (siladb.Database, error) {
	n.lock.Lock()
	defer n.lock.Unlock()
	if n.state == closedState {
		return nil, ErrNodeStopped
	}
	var db siladb.Database
	var err error
	if n.config.DataDir == "" {
...
func (n *Node) OpenDatabase(name string, cache, handles int, namespace string, readonly bool) (siladb.Database, error) {
...
// If the node has no data directory, an in-memory database is returned.
// Deprecated: use OpenDatabaseWithOptions instead.
func (n *Node) OpenDatabaseWithFreezer(name string, cache, handles int, ancient string, namespace string, readonly bool) (siladb.Database, error) {
	return n.OpenDatabaseWithOptions(name, DatabaseOptions{
...
func (n *Node) wrapDatabase(db siladb.Database) siladb.Database {
