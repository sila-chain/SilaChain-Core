...
// https://github.com/sila-org/consensus-specs/blob/dev/specs/altair/light-client/sync-protocol.md#lightclientupdate
...
func (api *BeaconLightApi) GetBestUpdatesAndCommittees(firstPeriod, count uint64) ([]*types.LightClientUpdate, []*types.SerializedSyncCommittee, error) {
	resp, err := api.httpGet("/sila/v1/beacon/light_client/updates", map[string][]string{
...
//
// See data structure definition here:
// https://github.com/sila-org/consensus-specs/blob/dev/specs/altair/light-client/sync-protocol.md#lightclientoptimisticupdate
func (api *BeaconLightApi) GetOptimisticUpdate() (types.OptimisticUpdate, error) {
	resp, err := api.httpGet("/sila/v1/beacon/light_client/optimistic_update", nil)
	if err != nil {
...
//
// See data structure definition here:
// https://github.com/sila-org/consensus-specs/blob/dev/specs/altair/light-client/sync-protocol.md#lightclientfinalityupdate
func (api *BeaconLightApi) GetFinalityUpdate() (types.FinalityUpdate, error) {
	resp, err := api.httpGet("/sila/v1/beacon/light_client/finality_update", nil)
	if err != nil {
...
func (api *BeaconLightApi) GetHeader(blockRoot common.Hash) (types.Header, bool, bool, error) {
	...
	resp, err := api.httpGet(fmt.Sprintf("/sila/v1/beacon/headers/%s", blockId), nil)
	if err != nil {
...
func (api *BeaconLightApi) GetCheckpointData(checkpointHash common.Hash) (*types.BootstrapData, error) {
	resp, err := api.httpGet(fmt.Sprintf("/sila/v1/beacon/light_client/bootstrap/0x%x", checkpointHash[:]), nil)
	if err != nil {
		return nil, err
	}

	// See data structure definition here:
	// https://github.com/sila-org/consensus-specs/blob/dev/specs/altair/light-client/sync-protocol.md#lightclientbootstrap
...
func (api *BeaconLightApi) GetBeaconBlock(blockRoot common.Hash) (*types.BeaconBlock, error) {
	resp, err := api.httpGet(fmt.Sprintf("/sila/v2/beacon/blocks/0x%x", blockRoot), nil)
	if err != nil {
		return nil, err
	}
...
func (api *BeaconLightApi) startEventStream(ctx context.Context, listener *HeadEventListener) *eventsource.Stream {
	for retry := true; retry; retry = ctxSleep(ctx, 5*time.Second) {
		...
		uri, err := api.buildURL("/sila/v1/events", map[string][]string{"topics": {"head", "light_client_finality_update", "light_client_optimistic_update"}})
		if err != nil {
