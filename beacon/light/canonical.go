...
func newCanonicalStore[T any](db siladb.Iteratee, keyPrefix []byte) (*canonicalStore[T], error) {
	cs := &canonicalStore[T]{
		keyPrefix: keyPrefix,
...
func (cs *canonicalStore[T]) add(backend siladb.KeyValueWriter, period uint64, value T) error {
...
func (cs *canonicalStore[T]) deleteFrom(db siladb.KeyValueWriter, fromPeriod uint64) (deleted periodRange) {
...
func (cs *canonicalStore[T]) get(backend siladb.KeyValueReader, period uint64) (T, bool) {
	var null, value T
