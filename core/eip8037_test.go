...
func TestBlockBaseFeeUsesMax(t *testing.T) {
	...
	_, blocks, _ := GenerateChainWithGenesis(env.gspec, silaEngine, 1, func(_ int, b *BlockGen) {
...
func TestReceiptCumulativeGasUsed(t *testing.T) {
	...
	silaEngine := beacon.New(silaash.NewFaker())
	_, _, receipts := GenerateChainWithGenesis(env.gspec, silaEngine, 1, func(_ int, g *BlockGen) {
