...
func testHeaderVerification(t *testing.T, scheme string) {
	...
	var (
		...
		_, blocks, _ = GenerateChainWithGenesis(gspec, silaash.NewFaker(), 8, nil)
	)
	...
	chain, err := NewBlockChain(rawdb.NewMemoryDatabase(), gspec, silaash.NewFaker(), options)
	defer chain.Stop()
	if err != nil {
		t.Fatal(err)
	}
	...
	for i := 0; i < len(blocks); i++ {
		for j, valid := range []bool{true, false} {
			...
			if valid {
				...
				_, results = silaEngine.VerifyHeaders(chain, []*types.Header{headers[i]})
			} else {
				silaEngine := silaash.NewFakeFailer(headers[i].Number.Uint64())
				_, results = silaEngine.VerifyHeaders(chain, []*types.Header{headers[i]})
			}
...
func TestHeaderVerificationForMergingSilaash(t *testing.T) { testHeaderVerificationForMerging(t, false) }
...
func testHeaderVerificationForMerging(t *testing.T, isClique bool) {
	...
	if isClique {
		...
		silaEngine = beacon.New(clique.New(params.AllCliqueProtocolChanges.Clique, rawdb.NewMemoryDatabase()))
		gspec = &Genesis{
			Config:    &config,
		...
		td := 0
		genDb, blocks, _ := GenerateChainWithGenesis(gspec, silaEngine, 8, nil)
		...
		for i, block := range blocks {
			...
			sig, _ := crypto.Sign(silaEngine.SealHash(header).Bytes(), key)
		...
		postBlocks, _ = GenerateChain(gspec.Config, preBlocks[len(preBlocks)-1], silaEngine, genDb, 8, nil)
	} else {
		...
		preBlocks = blocks
		gspec.Config.TerminalTotalDifficulty = big.NewInt(int64(td))
		postBlocks, _ = GenerateChain(gspec.Config, preBlocks[len(preBlocks)-1], silaEngine, genDb, 8, func(i int, gen *BlockGen) {
			gen.SetPoS()
		})
	}
	...
	for i := 0; i < len(preBlocks); i++ {
		_, results := silaEngine.VerifyHeaders(chain, []*types.Header{preHeaders[i]})
		// Wait for the verification result
		select {
		case result := <-results:
			if result != nil {
	...
	for i := 0; i < len(postBlocks); i++ {
		_, results := silaEngine.VerifyHeaders(chain, []*types.Header{postHeaders[i]})
		// Wait for the verification result
		select {
		case result := <-results:
			if result != nil {
	...
	_, results := silaEngine.VerifyHeaders(chain, headers)
