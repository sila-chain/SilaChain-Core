...
// silaash tests, please use OffsetTime, which implicitly recalculates the diff.
...
func (b *BlockGen) AddUncle(h *types.Header) {
	...
	h.Difficulty = b.silaEngine.CalcDifficulty(b.cm, b.header.Time, parent)
...
func GenerateChain(config *params.ChainConfig, parent *types.Block, silaEngine consensus.SilaEngine, db siladb.Database, n int, gen func(int, *BlockGen)) ([]*types.Block, []types.Receipts) {
	if config == nil {
		config = params.TestChainConfig
	}
	if silaEngine == nil {
		panic("nil consensus silaEngine")
	}
	cm := newChainMaker(parent, config, silaEngine)

	genblock := func(i int, parent *types.Block, triedb *triedb.Database, statedb *state.StateDB) (*types.Block, types.Receipts) {
		b := &BlockGen{i: i, cm: cm, parent: parent, statedb: statedb, silaEngine: silaEngine}
		b.header = cm.makeHeader(parent, statedb, b.silaEngine)
		...
		b.silaEngine.Finalize(cm, b.header, statedb, &body, uint32(len(b.txs)+1), b.bal)
...
func GenerateChainWithGenesis(genesis *Genesis, silaEngine consensus.SilaEngine, n int, gen func(int, *BlockGen)) (siladb.Database, []*types.Block, []types.Receipts) {
	...
	genesisTriedb.Close()
	blocks, receipts := GenerateChain(genesis.Config, block, silaEngine, db, n, gen)
	return db, blocks, receipts
}

func (cm *chainMaker) makeHeader(parent *types.Block, state *state.StateDB, silaEngine consensus.SilaEngine) *types.Header {
...
func makeHeaderChain(chainConfig *params.ChainConfig, parent *types.Header, n int, silaEngine consensus.SilaEngine, db siladb.Database, seed int) []*types.Header {
	blocks := makeBlockChain(chainConfig, types.NewBlockWithHeader(parent), n, silaEngine, db, seed)
...
func makeHeaderChainWithGenesis(genesis *Genesis, n int, silaEngine consensus.SilaEngine, seed int) (siladb.Database, []*types.Header) {
	db, blocks := makeBlockChainWithGenesis(genesis, n, silaEngine, seed)
	headers := make([]*types.Header, len(blocks))
	for i, block := range blocks {
		headers[i] = block.Header()
	}
...
func makeBlockChain(chainConfig *params.ChainConfig, parent *types.Block, n int, silaEngine consensus.SilaEngine, db siladb.Database, seed int) []*types.Block {
	blocks, _ := GenerateChain(chainConfig, parent, silaEngine, db, n, func(i int, b *BlockGen) {
...
func makeBlockChainWithGenesis(genesis *Genesis, n int, silaEngine consensus.SilaEngine, seed int) (siladb.Database, []*types.Block) {
	db, blocks, _ := GenerateChainWithGenesis(genesis, silaEngine, n, func(i int, b *BlockGen) {
		b.SetCoinbase(common.Address{0: byte(seed), 19: byte(i)})
	})
	return db, blocks
}
...
func newChainMaker(bottom *types.Block, config *params.ChainConfig, silaEngine consensus.SilaEngine) *chainMaker {
	return &chainMaker{
		bottom:      bottom,
		config:      config,
...
// SilaEngine returns the consensus silaEngine (for ChainContext).
func (cm *chainMaker) SilaEngine() consensus.SilaEngine {
	return cm.silaEngine
}
