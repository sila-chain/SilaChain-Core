...
func init() {
	...
	sip1559Config = &cpy
	sip1559Config.BerlinBlock = common.Big0
	sip1559Config.LondonBlock = common.Big0
}
...
func (c *testChain) State() (*state.StateDB, error) {
	...
	if *c.trigger {
		...
		c.statedb.SetBalance(c.address, new(uint256.Int).SetUint64(params.SilaBaseUnit), tracing.BalanceChangeUnspecified)
		*c.trigger = false
	}
	return stdb, nil
}
...
func TestStateChangeDuringReset(t *testing.T) {
	...
	statedb.SetBalance(address, new(uint256.Int).SetUint64(params.SilaBaseUnit), tracing.BalanceChangeUnspecified)
...
func TestSetCodeTransactions(t *testing.T) {
	...
	testAddBalance(pool, addrA, big.NewInt(params.SilaBaseUnit))
	testAddBalance(pool, addrB, big.NewInt(params.SilaBaseUnit))
	testAddBalance(pool, addrC, big.NewInt(params.SilaBaseUnit))
		...
		},
		{
			...
			run: func(name string) {
				...
				for i := 0; i < 30; i++ {
					...
					addr := crypto.PubkeyToAddress(key.PublicKey)
					testAddBalance(pool, addr, big.NewInt(params.SilaBaseUnit))
				}
