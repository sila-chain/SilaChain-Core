...
func TestProcessUBT(t *testing.T) {
	...
	blockchain, _ := NewBlockChain(bcdb, gspec, beacon.New(silaash.NewFaker()), options)
	...
	_, chain, _ := GenerateChainWithGenesis(gspec, beacon.New(silaash.NewFaker()), 2, func(i int, gen *BlockGen) {
		gen.SetPoS()
