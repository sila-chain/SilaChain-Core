...
func ReadDatabaseVersion(db siladb.KeyValueReader) *uint64 {
	var version uint64
...
func WriteDatabaseVersion(db siladb.KeyValueWriter, version uint64) {
...
func ReadChainConfig(db siladb.KeyValueReader, hash common.Hash) *params.ChainConfig {
...
func WriteChainConfig(db siladb.KeyValueWriter, hash common.Hash, cfg *params.ChainConfig) {
	if cfg == nil {
		return
	}
...
func ReadGenesisStateSpec(db siladb.KeyValueReader, blockhash common.Hash) []byte {
	data, _ := db.Get(genesisStateSpecKey(blockhash))
	return data
}

// WriteGenesisStateSpec writes the genesis state specification into the disk.
func WriteGenesisStateSpec(db siladb.KeyValueWriter, blockhash common.Hash, data []byte) {
...
func PushUncleanShutdownMarker(db siladb.KeyValueStore) ([]uint64, uint64, error) {
...
func PopUncleanShutdownMarker(db siladb.KeyValueStore) {
	var uncleanShutdowns crashList
	// Read old data
	if data, err := db.Get(uncleanShutdownKey); err != nil {
...
func UpdateUncleanShutdownMarker(db siladb.KeyValueStore) {
	var uncleanShutdowns crashList
	// Read old data
	if data, err := db.Get(uncleanShutdownKey); err != nil {
		log.Warn("Error reading unclean shutdown markers", "error", err)
