...
func ReadSkeletonSyncStatus(db siladb.KeyValueReader) []byte {
	data, _ := db.Get(skeletonSyncStatusKey)
	return data
}

// WriteSkeletonSyncStatus stores the serialized sync status to save at shutdown.
func WriteSkeletonSyncStatus(db siladb.KeyValueWriter, status []byte) {
...
// shutdown
func DeleteSkeletonSyncStatus(db siladb.KeyValueWriter) {
...
func ReadSkeletonHeader(db siladb.KeyValueReader, number uint64) *types.Header {
...
func WriteSkeletonHeader(db siladb.KeyValueWriter, header *types.Header) {
...
func DeleteSkeletonHeader(db siladb.KeyValueWriter, number uint64) {
...
func ReadSnapSyncStatusFlag(db siladb.KeyValueReader) uint8 {
...
func WriteSnapSyncStatusFlag(db siladb.KeyValueWriter, flag uint8) {
