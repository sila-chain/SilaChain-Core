...
//     state freezer.
func NewStateFreezer(ancientDir string, verkle bool, readOnly bool) (siladb.ResettableAncientStore, error) {
	if ancientDir == "" {
...
//   - if non-empty directory is given, initializes the regular file-based
//     trienode freezer.
func NewTrienodeFreezer(ancientDir string, verkle bool, readOnly bool) (siladb.ResettableAncientStore, error) {
	if ancientDir == "" {
