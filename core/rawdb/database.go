...
func (db *nofreezedb) ModifyAncients(func(siladb.AncientWriteOp) error) (int64, error) {
	return 0, errNotSupported
}
...
func (db *nofreezedb) ReadAncients(fn func(reader siladb.AncientReaderOp) error) (err error) {
...
func NewDatabase(db siladb.KeyValueStore) siladb.Database {
...
//
// Deprecated: use Open.
func NewDatabaseWithFreezer(db siladb.KeyValueStore, ancient string, namespace string, readonly bool) (siladb.Database, error) {
...
func Open(db siladb.KeyValueStore, opts OpenOptions) (siladb.Database, error) {
...
func NewMemoryDatabase() siladb.Database {
...
func InspectDatabase(db siladb.Database, keyPrefix, keyStart []byte) error {
	var (
		start = time.Now()
		count atomic.Int64
		total atomic.Uint64
...
func printChainMetadata(db siladb.KeyValueStore) {
...
func ReadChainMetadata(db siladb.KeyValueStore) [][]string {
...
// deleted already.
func SafeDeleteRange(db siladb.KeyValueStore, start, end []byte, hashScheme bool, stopCallback func(bool) bool) error {
	if !hashScheme {
		...
		for {
			...
			case err == nil:
				return nil
			case errors.Is(err, siladb.ErrTooManyKeys):
