...
func (f *Freezer) ReadAncients(fn func(siladb.AncientReaderOp) error) (err error) {
	f.writeLock.RLock()
	defer f.writeLock.RUnlock()
...
func (f *Freezer) ModifyAncients(fn func(siladb.AncientWriteOp) error) (writeSize int64, err error) {
	if f.readonly {
