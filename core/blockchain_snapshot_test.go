...
func (basic *snapshotTestBasic) prepare(t *testing.T) (*BlockChain, []*types.Block) {
	...
	chain, err := NewBlockChain(db, gspec, silaEngine, DefaultConfig().WithStateScheme(basic.scheme).WithNoAsyncFlush(true))
	if err != nil {
		t.Fatalf("Failed to create chain: %v", err)
	}
	genDb, blocks, _ := GenerateChainWithGenesis(gspec, silaEngine, basic.chainBlocks, func(i int, b *BlockGen) {})
...
func (snaptest *snapshotTest) test(t *testing.T) {
	...
	chain.Stop()
	newchain, err := NewBlockChain(snaptest.db, snaptest.gspec, snaptest.silaEngine, DefaultConfig().WithStateScheme(snaptest.scheme))
	if err != nil {
...
func (snaptest *crashSnapshotTest) test(t *testing.T) {
	...
	newchain, err := NewBlockChain(newdb, snaptest.gspec, snaptest.silaEngine, DefaultConfig().WithStateScheme(snaptest.scheme))
	if err != nil {
		t.Fatalf("Failed to recreate chain: %v", err)
	}
	newchain.Stop()
...
func (snaptest *gappedSnapshotTest) test(t *testing.T) {
	...
	chain.Stop()
	gappedBlocks, _ := GenerateChain(snaptest.gspec.Config, blocks[len(blocks)-1], snaptest.silaEngine, snaptest.genDb, snaptest.gapped, func(i int, b *BlockGen) {})
	...
	newchain, err := NewBlockChain(snaptest.db, snaptest.gspec, snaptest.silaEngine, options)
	if err != nil {
		t.Fatalf("Failed to recreate chain: %v", err)
	}
...
func (snaptest *wipeCrashSnapshotTest) test(t *testing.T) {
	...
	newchain, err := NewBlockChain(snaptest.db, snaptest.gspec, snaptest.silaEngine, config)
	if err != nil {
		t.Fatalf("Failed to recreate chain: %v", err)
	}
	newBlocks, _ := GenerateChain(snaptest.gspec.Config, blocks[len(blocks)-1], snaptest.silaEngine, snaptest.genDb, snaptest.newBlocks, func(i int, b *BlockGen) {})
	...
	tmp, err := NewBlockChain(snaptest.db, snaptest.gspec, snaptest.silaEngine, config)
	if err != nil {
		t.Fatalf("Failed to recreate chain: %v", err)
	}
...
func TestSnapSyncCompleteRebuildsSnapshot(t *testing.T) {
	for _, isSnapV2 := range []bool{false, true} {
		_, _, chain, err := newCanonical(silaash.NewFaker(), 8, true, rawdb.HashScheme)
		if err != nil {
			t.Fatalf("failed to create chain: %v", err)
		}
