...
func TestStateProcessorErrors(t *testing.T) {
	...
	{ // Tests against a 'recent' chain definition
		var (
			...
			blockchain, _  = NewBlockChain(db, gspec, beacon.New(silaash.NewFaker()), nil)
		...
		for i, tt := range []struct {
			...
			block := GenerateBadBlock(gspec.ToBlock(), beacon.New(silaash.NewFaker()), tt.txs, gspec.Config, false)
	...
	// ErrTxTypeNotSupported, For this, we need an older chain
	{
		var (
			...
			blockchain, _ = NewBlockChain(db, gspec, silaash.NewFaker(), nil)
		)
		defer blockchain.Stop()
		for i, tt := range []struct {
			txs  []*types.Transaction
			...
			block := GenerateBadBlock(gspec.ToBlock(), silaash.NewFaker(), tt.txs, gspec.Config, true)
			_, err := blockchain.InsertChain(types.Blocks{block})
			if err == nil {
				t.Fatal("block imported without errors")
			}
...
func GenerateBadBlock(parent *types.Block, silaEngine consensus.SilaEngine, txs types.Transactions, config *params.ChainConfig, isPOW bool) *types.Block {
	difficulty := big.NewInt(0)
	if isPOW {
		fakeChainReader := newChainMaker(nil, config, silaEngine)
		difficulty = silaEngine.CalcDifficulty(fakeChainReader, parent.Time()+10, &types.Header{
