...
func TestGeneratePOSChain(t *testing.T) {
	var (
		...
		funds   = big.NewInt(0).Mul(big.NewInt(1337), big.NewInt(params.SilaBaseUnit))
	...
	genchain, genreceipts := GenerateChain(gspec.Config, genesis, silaEngine, gendb, 4, func(i int, gen *BlockGen) {
	...
	blockchain, _ := NewBlockChain(db, gspec, silaEngine, nil)
...
func ExampleGenerateChain() {
	...
	chain, _ := GenerateChain(gspec.Config, genesis, silaash.NewFaker(), genDb, 5, func(i int, gen *BlockGen) {
		switch i {
		case 0:
	...
	// Import the chain. This runs all block validation rules.
	blockchain, _ := NewBlockChain(db, gspec, silaash.NewFaker(), DefaultConfig().WithStateScheme(rawdb.HashScheme))
	defer blockchain.Stop()
