...
func ExampleGenerateChain() {
	...
	chain, _ := GenerateChain(gspec.Config, genesis, silapow.NewSilaPoWFaker(), genDb, 5, func(i int, gen *BlockGen) {
		switch i {
		case 0:
	...
	// Import the chain. This runs all block validation rules.
	blockchain, _ := NewBlockChain(db, gspec, silapow.NewSilaPoWFaker(), DefaultConfig().WithStateScheme(rawdb.HashScheme))
	defer blockchain.Stop()
