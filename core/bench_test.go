...
func benchInsertChain(b *testing.B, disk bool, gen func(int, *BlockGen)) {
	...
	_, chain, _ := GenerateChainWithGenesis(gspec, silaash.NewFaker(), b.N, gen)

	// Time the insertion of the new chain.
	// State and blocks are stored in the same DB.
	chainman, _ := NewBlockChain(db, gspec, silaash.NewFaker(), nil)
	defer chainman.Stop()
	b.ReportAllocs()
	b.ResetTimer()
...
// into a database.
func makeChainForBench(db siladb.Database, genesis *Genesis, full bool, count uint64) {
...
func benchWriteChain(b *testing.B, full bool, count uint64) {
	genesis := &Genesis{Config: params.AllSilaashProtocolChanges}
	for b.Loop() {
...
func benchReadChain(b *testing.B, full bool, count uint64) {
	...
	for b.Loop() {
		...
		chain, err := NewBlockChain(db, genesis, silaash.NewFaker(), options)
		if err != nil {
