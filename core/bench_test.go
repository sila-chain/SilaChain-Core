...
func benchInsertChain(b *testing.B, disk bool, gen func(int, *BlockGen)) {
	...
	_, chain, _ := GenerateChainWithGenesis(gspec, silapow.NewSilaPoWFaker(), b.N, gen)

	// Time the insertion of the new chain.
	// State and blocks are stored in the same DB.
	chainman, _ := NewBlockChain(db, gspec, silapow.NewSilaPoWFaker(), nil)
	defer chainman.Stop()
	b.ReportAllocs()
	b.ResetTimer()
...
func benchReadChain(b *testing.B, full bool, count uint64) {
	...
	for b.Loop() {
		...
		chain, err := NewBlockChain(db, genesis, silapow.NewSilaPoWFaker(), options)
		if err != nil {
