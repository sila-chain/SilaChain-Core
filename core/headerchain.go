...
func NewHeaderChain(chainDb siladb.Database, config *params.ChainConfig, silaEngine consensus.SilaEngine, procInterrupt func() bool) (*HeaderChain, error) {
	hc := &HeaderChain{
		config:        config,
		chainDb:       chainDb,
...
func (hc *HeaderChain) ValidateHeaderChain(chain []*types.Header) (int, error) {
	...
	abort, results := hc.silaEngine.VerifyHeaders(hc, chain)
	defer close(abort)
...
type (
	...
	UpdateHeadBlocksCallback func(siladb.KeyValueWriter, *types.Header) (*types.Header, bool)

	// DeleteBlockContentCallback is a callback function that is called by SetHead
	// before each header is deleted.
	DeleteBlockContentCallback func(siladb.KeyValueWriter, common.Hash, uint64)
)
...
// SilaEngine retrieves the header chain's consensus silaEngine.
func (hc *HeaderChain) SilaEngine() consensus.SilaEngine { return hc.silaEngine }
