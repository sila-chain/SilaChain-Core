...
func (tc *testChain) addBlocks(count, maxTxPerBlock, maxLogsPerReceipt, maxTopicsPerLog int, random bool) {
	...
	if len(tc.canonical) == 0 {
		...
		tc.db, blocks, receipts = core.GenerateChainWithGenesis(gspec, silaEngine, count, blockGen)
		...
		blocks, receipts = core.GenerateChain(params.TestChainConfig, tc.blocks[tc.canonical[len(tc.canonical)-1]], silaEngine, tc.db, count, blockGen)
	}
