...
// invoked by the sila core at various points in the state transition.
//
// To create a tracer that can be invoked with Sila, you need to register it using
// [github.com/sila-org/sila/sila/tracers.LiveDirectory.Register].
//
...
const (
	...
	// BalanceIncreaseGasReturn is sila returned for unused gas at the end of execution.
	...
	// BalanceDecreaseDaoAccount is sila taken from a DAO account to be moved to the refund contract.
	BalanceDecreaseDaoAccount BalanceChangeReason = 9

	// BalanceChangeTransfer is sila transferred via a call.
	...
	// BalanceDecreaseSelfdestructBurn is sila that is sent to an already self-destructed
