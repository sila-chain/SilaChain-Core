...
func newCanonical(silaEngine consensus.SilaEngine, n int, full bool, scheme string) (siladb.Database, *Genesis, *BlockChain, error) {
	var (
		genesis = &Genesis{
	...
	blockchain, _ := NewBlockChain(rawdb.NewMemoryDatabase(), genesis, silaEngine, options)
	...
	if full {
		...
		genDb, blocks := makeBlockChainWithGenesis(genesis, n, silaEngine, canonicalSeed)
		_, err := blockchain.InsertChain(blocks)
		return genDb, genesis, blockchain, err
	}
	// Header-only chain requested
	genDb, headers := makeHeaderChainWithGenesis(genesis, n, silaEngine, canonicalSeed)
...
func testFork(t *testing.T, blockchain *BlockChain, i, n int, full bool, scheme string) {
	...
	genDb, _, blockchain2, err := newCanonical(silaash.NewFaker(), i, full, scheme)
	if err != nil {
	...
	if full {
		blockChainB = makeBlockChain(blockchain2.chainConfig, blockchain2.GetBlockByHash(blockchain2.CurrentBlock().Hash()), n, silaash.NewFaker(), genDb, forkSeed)
		if _, err := blockchain2.InsertChain(blockChainB); err != nil {
			t.Fatalf("failed to insert forking chain: %v", err)
		}
	} else {
		headerChainB = makeHeaderChain(blockchain2.chainConfig, blockchain2.CurrentHeader(), n, silaash.NewFaker(), genDb, forkSeed)
...
func testBlockChainImport(chain types.Blocks, blockchain *BlockChain) error {
	for _, block := range chain {
		...
		err := blockchain.silaEngine.VerifyHeader(blockchain, block.Header())
		if err == nil {
...
func testHeaderChainImport(chain []*types.Header, blockchain *BlockChain) error {
	for _, header := range chain {
		...
		if err := blockchain.silaEngine.VerifyHeader(blockchain, header); err != nil {
			return err
		}
...
func testLastBlock(t *testing.T, scheme string) {
	genDb, _, blockchain, err := newCanonical(silaash.NewFaker(), 0, true, scheme)
	if err != nil {
		t.Fatalf("failed to create pristine chain: %v", err)
	}
	defer blockchain.Stop()

	blocks := makeBlockChain(blockchain.chainConfig, blockchain.GetBlockByHash(blockchain.CurrentBlock().Hash()), 1, silaash.NewFullFaker(), genDb, 0)
...
func testExtendCanonical(t *testing.T, full bool, scheme string) {
	...
	_, _, processor, err := newCanonical(silaash.NewFaker(), length, full, scheme)
	if err != nil {
...
func testBrokenChain(t *testing.T, full bool, scheme string) {
	...
	genDb, _, blockchain, err := newCanonical(silaash.NewFaker(), 10, full, scheme)
	if err != nil {
		t.Fatalf("failed to make new canonical chain: %v", err)
	}
	defer blockchain.Stop()
	...
	if full {
		chain := makeBlockChain(blockchain.chainConfig, blockchain.GetBlockByHash(blockchain.CurrentBlock().Hash()), 5, silaash.NewFaker(), genDb, forkSeed)[1:]
		if err := testBlockChainImport(chain, blockchain); err == nil {
			t.Errorf("broken block chain not reported")
		}
	} else {
		chain := makeHeaderChain(blockchain.chainConfig, blockchain.CurrentHeader(), 5, silaash.NewFaker(), genDb, forkSeed)[1:]
...
func testReorg(t *testing.T, first, second []int64, td int64, full bool, scheme string) {
	...
	genDb, _, blockchain, err := newCanonical(silaash.NewFaker(), 0, full, scheme)
	if err != nil {
		t.Fatalf("failed to create pristine chain: %v", err)
	}
	defer blockchain.Stop()
	...
	easyBlocks, _ := GenerateChain(params.TestChainConfig, blockchain.GetBlockByHash(blockchain.CurrentBlock().Hash()), silaash.NewFaker(), genDb, len(first), func(i int, b *BlockGen) {
		b.OffsetTime(first[i])
	})
	diffBlocks, _ := GenerateChain(params.TestChainConfig, blockchain.GetBlockByHash(blockchain.CurrentBlock().Hash()), silaash.NewFaker(), genDb, len(second), func(i int, b *BlockGen) {
...
func testInsertNonceError(t *testing.T, full bool, scheme string) {
	doTest := func(i int) {
		...
		if full {
			blocks := makeBlockChain(blockchain.chainConfig, blockchain.GetBlockByHash(blockchain.CurrentBlock().Hash()), i, silaash.NewFaker(), genDb, 0)

			failAt = rand.Int() % len(blocks)
			failNum = blocks[failAt].NumberU64()

			blockchain.silaEngine = silaash.NewFakeFailer(failNum)
			failRes, err = blockchain.InsertChain(blocks)
		} else {
			headers := makeHeaderChain(blockchain.chainConfig, blockchain.CurrentHeader(), i, silaash.NewFaker(), genDb, 0)

			failAt = rand.Int() % len(headers)
			failNum = headers[failAt].Number.Uint64()

			blockchain.silaEngine = silaash.NewFakeFailer(failNum)
			blockchain.hc.silaEngine = blockchain.silaEngine
...
func testFastVsFullChains(t *testing.T, scheme string) {
	...
	_, blocks, receipts := GenerateChainWithGenesis(gspec, silaash.NewFaker(), 1024, func(i int, block *BlockGen) {
	...
	archive, _ := NewBlockChain(archiveDb, gspec, silaash.NewFaker(), DefaultConfig().WithStateScheme(scheme))
	defer archive.Stop()
	...
	fast, _ := NewBlockChain(fastDb, gspec, silaash.NewFaker(), DefaultConfig().WithStateScheme(scheme))
	defer fast.Stop()
	...
	ancient, _ := NewBlockChain(ancientDb, gspec, silaash.NewFaker(), DefaultConfig().WithStateScheme(scheme))
	defer ancient.Stop()
...
func testLightVsFastVsFullChainHeads(t *testing.T, scheme string) {
	...
	height := uint64(64)
	_, blocks, receipts := GenerateChainWithGenesis(gspec, silaash.NewFaker(), int(height), nil)

	// makeDb creates a db instance for testing.
	makeDb := func() siladb.Database {
	...
	archive, _ := NewBlockChain(archiveDb, gspec, silaash.NewFaker(), options)
	if n, err := archive.InsertChain(blocks); err != nil {
		t.Fatalf("failed to process block %d: %v", n, err)
	}
	defer archive.Stop()
	...
	light, _ := NewBlockChain(lightDb, gspec, silaash.NewFaker(), DefaultConfig().WithStateScheme(scheme))
...
func testChainTxReorgs(t *testing.T, scheme string) {
	...
	_, chain, _ := GenerateChainWithGenesis(gspec, silaash.NewFaker(), 3, func(i int, gen *BlockGen) {
		switch i {
		case 0:
	...
	blockchain, _ := NewBlockChain(db, gspec, silaash.NewFaker(), DefaultConfig().WithStateScheme(scheme))
	...
	_, chain, _ = GenerateChainWithGenesis(gspec, silaash.NewFaker(), 5, func(i int, gen *BlockGen) {
		switch i {
		case 0:
...
func testLogReorgs(t *testing.T, scheme string) {
	...
	_, chain, _ := GenerateChainWithGenesis(gspec, silaash.NewFaker(), 2, func(i int, gen *BlockGen) {
		if i == 1 {
...
func testLogRebirth(t *testing.T, scheme string) {
	...
	genDb, chain, _ := GenerateChainWithGenesis(gspec, silaEngine, 3, func(i int, gen *BlockGen) {
		if i < 2 {
			for ii := 0; ii < 5; ii++ {
	...
	_, forkChain, _ := GenerateChainWithGenesis(gspec, silaEngine, 3, func(i int, gen *BlockGen) {
		if i == 2 {
	...
	newBlocks, _ := GenerateChain(gspec.Config, chain[len(chain)-1], silaEngine, genDb, 1, func(i int, gen *BlockGen) {})
...
func testSideLogRebirth(t *testing.T, scheme string) {
	...
	genDb, sideChain, _ := GenerateChainWithGenesis(gspec, silaash.NewFaker(), 2, func(i int, gen *BlockGen) {
		if i == 1 {
	...
	newBlocks, _ := GenerateChain(gspec.Config, sideChain[len(sideChain)-1], silaash.NewFaker(), genDb, 1, func(i int, gen *BlockGen) {})
	if _, err := blockchain.InsertChain(newBlocks); err != nil {
		t.Fatalf("failed to insert forked chain: %v", err)
	}
	checkLogEvents(t, newLogCh, rmLogsCh, 0, 0)
...
func testCanonicalBlockRetrieval(t *testing.T, scheme string) {
	_, gspec, blockchain, err := newCanonical(silaash.NewFaker(), 0, true, scheme)
	if err != nil {
		t.Fatalf("failed to create pristine chain: %v", err)
	}
	defer blockchain.Stop()

	_, chain, _ := GenerateChainWithGenesis(gspec, silaash.NewFaker(), 10, func(i int, gen *BlockGen) {})
...
func testSIP155Transition(t *testing.T, scheme string) {
	...
	genDb, blocks, _ := GenerateChainWithGenesis(gspec, silaash.NewFaker(), 4, func(i int, block *BlockGen) {
		var (
...
func testSIP161AccountRemoval(t *testing.T, scheme string) {
	...
	_, blocks, _ := GenerateChainWithGenesis(gspec, silaash.NewFaker(), 3, func(i int, block *BlockGen) {
		var (
			tx     *types.Transaction
			err    error
			signer = types.LatestSigner(gspec.Config)
...
func testBlockchainHeaderchainReorgConsistency(t *testing.T, scheme string) {
	...
	genDb, blocks, _ := GenerateChainWithGenesis(genesis, silaEngine, 64, func(i int, b *BlockGen) { b.SetCoinbase(common.Address{1}) })
	...
	for i := 0; i < len(forks); i++ {
		...
		fork, _ := GenerateChain(genesis.Config, parent, silaEngine, genDb, 1, func(i int, b *BlockGen) { b.SetCoinbase(common.Address{2}) })
		forks[i] = fork[0]
	}
	// Import the canonical and fork chain side by side, verifying the current block
	// and current header consistency
	chain, err := NewBlockChain(rawdb.NewMemoryDatabase(), genesis, silaEngine, DefaultConfig().WithStateScheme(scheme))
	if err != nil {
...
func TestTrieForkGC(t *testing.T) {
	...
	genDb, blocks, _ := GenerateChainWithGenesis(genesis, silaEngine, 2*state.TriesInMemory, func(i int, b *BlockGen) { b.SetCoinbase(common.Address{1}) })
	...
	chain, err := NewBlockChain(rawdb.NewMemoryDatabase(), genesis, silaEngine, nil)
	if err != nil {
		t.Fatalf("failed to create tester chain: %v", err)
	}
	defer chain.Stop()
...
func testLargeReorgTrieGC(t *testing.T, scheme string) {
	...
	genDb, shared, _ := GenerateChainWithGenesis(genesis, silaEngine, 64, func(i int, b *BlockGen) { b.SetCoinbase(common.Address{1}) })
	original, _ := GenerateChain(genesis.Config, shared[len(shared)-1], silaEngine, genDb, 2*state.TriesInMemory, func(i int, b *BlockGen) { b.SetCoinbase(common.Address{2}) })
	competitor, _ := GenerateChain(genesis.Config, shared[len(shared)-1], silaEngine, genDb, 2*state.TriesInMemory+1, func(i int, b *BlockGen) { b.SetCoinbase(common.Address{3}) })

	// Import the shared chain and the original canonical one
	db, _ := rawdb.Open(rawdb.NewMemoryDatabase(), rawdb.OpenOptions{})
	defer db.Close()

	chain, err := NewBlockChain(db, genesis, silaEngine, DefaultConfig().WithStateScheme(scheme))
	if err != nil {
		t.Fatalf("failed to create tester chain: %v", err)
	}
	defer chain.Stop()
...
func testLowDiffLongChain(t *testing.T, scheme string) {
	...
	genDb, blocks, _ := GenerateChainWithGenesis(genesis, silaEngine, 6*state.TriesInMemory, func(i int, b *BlockGen) {
	...
	chain, err := NewBlockChain(diskdb, genesis, silaEngine, DefaultConfig().WithStateScheme(scheme))
	if err != nil {
		t.Fatalf("failed to create tester chain: %v", err)
	}
	defer chain.Stop()
	...
	parent := blocks[10]
	fork, _ := GenerateChain(genesis.Config, parent, silaEngine, genDb, 8*state.TriesInMemory, func(i int, b *BlockGen) {
...
func testSideImport(t *testing.T, numCanonBlocksInSidechain, blocksBetweenCommonAncestorAndPruneblock int, mergePoint int) {
	...
	chain, err := NewBlockChain(rawdb.NewMemoryDatabase(), gspec, silaEngine, nil)
	if err != nil {
		t.Fatalf("failed to create tester chain: %v", err)
	}
	defer chain.Stop()
	...
	genDb, blocks, _ := GenerateChainWithGenesis(gspec, silaEngine, 2*state.TriesInMemory, func(i int, gen *BlockGen) {
	...
	fork, _ := GenerateChain(gspec.Config, parent, silaEngine, genDb, 2*state.TriesInMemory, func(i int, b *BlockGen) {
		b.SetCoinbase(common.Address{2})
...
func testInsertKnownChainData(t *testing.T, typ string, scheme string) {
	...
	genDb, blocks, receipts := GenerateChainWithGenesis(genesis, silaEngine, 32, func(i int, b *BlockGen) { b.SetCoinbase(common.Address{1}) })

	// A longer chain but total difficulty is lower.
	blocks2, receipts2 := GenerateChain(genesis.Config, blocks[len(blocks)-1], silaEngine, genDb, 65, func(i int, b *BlockGen) { b.SetCoinbase(common.Address{1}) })

	// A shorter chain but total difficulty is higher.
	blocks3, receipts3 := GenerateChain(genesis.Config, blocks[len(blocks)-1], silaEngine, genDb, 64, func(i int, b *BlockGen) {
		b.SetCoinbase(common.Address{1})
	...
	chain, err := NewBlockChain(chaindb, genesis, silaEngine, DefaultConfig().WithStateScheme(scheme))
	if err != nil {
		t.Fatalf("failed to create tester chain: %v", err)
	}
	defer chain.Stop()
...
func testInsertKnownChainDataWithMerging(t *testing.T, typ string, mergeHeight int) {
	...
	genDb, blocks, receipts := GenerateChainWithGenesis(genesis, silaEngine, 32,
	...
	blocks2, receipts2 := GenerateChain(genesis.Config, blocks[len(blocks)-1], silaEngine, genDb, 65, func(i int, b *BlockGen) {
		b.SetCoinbase(common.Address{1})
		if b.header.Number.Uint64() >= mergeBlock {
			b.SetPoS()
		}
	...
	chain, err := NewBlockChain(chaindb, genesis, silaEngine, nil)
	if err != nil {
		t.Fatalf("failed to create tester chain: %v", err)
	}
	defer chain.Stop()
...
func getLongAndShortChains(scheme string) (*BlockChain, []*types.Block, []*types.Block, *Genesis, error) {
	...
	genDb, longChain, _ := GenerateChainWithGenesis(genesis, silaEngine, 80, func(i int, b *BlockGen) {
		b.SetCoinbase(common.Address{1})
	})
	chain, err := NewBlockChain(rawdb.NewMemoryDatabase(), genesis, silaEngine, DefaultConfig().WithStateScheme(scheme))
	if err != nil {
	...
	heavyChainExt, _ := GenerateChain(genesis.Config, parent, silaEngine, genDb, 75, func(i int, b *BlockGen) {
		b.SetCoinbase(common.Address{2})
		b.OffsetTime(-9)
	})
...
func benchmarkLargeNumberOfValueToNonexisting(b *testing.B, numTxs, numBlocks int, recipientFn func(uint64) common.Address) {
	...
	_, shared, _ := GenerateChainWithGenesis(gspec, silaEngine, numBlocks, blockGenerator)
	b.StopTimer()
	b.ResetTimer()
	for i := 0; i < b.N; i++ {
		// Import the shared chain and the original canonical one
...
func testSideImportPrunedBlocks(t *testing.T, scheme string) {
	...
	// Generate and import the canonical chain
	_, blocks, _ := GenerateChainWithGenesis(genesis, silaEngine, 2*state.TriesInMemory, nil)
...
func testDeleteCreateRevert(t *testing.T, scheme string) {
	...
	_, blocks, _ := GenerateChainWithGenesis(gspec, silaEngine, 1, func(i int, b *BlockGen) {
		b.SetCoinbase(common.Address{1})
	...
	// Import the canonical chain
	options := DefaultConfig().WithStateScheme(scheme)
	chain, err := NewBlockChain(rawdb.NewMemoryDatabase(), gspec, silaEngine, options)
	if err != nil {
		t.Fatalf("failed to create tester chain: %v", err)
	}
	defer chain.Stop()
...
func testDeleteRecreateSlotsAcrossManyBlocks(t *testing.T, scheme string) {
	...
	_, blocks, _ := GenerateChainWithGenesis(gspec, silaEngine, 150, func(i int, b *BlockGen) {
...
func testInitThenFailCreateContract(t *testing.T, scheme string) {
	...
	nonce := uint64(0)
	_, blocks, _ := GenerateChainWithGenesis(gspec, silaEngine, 4, func(i int, b *BlockGen) {
		b.SetCoinbase(common.Address{1})
...
func testSIP1559Transition(t *testing.T, scheme string) {
	...
	genDb, blocks, _ := GenerateChainWithGenesis(gspec, silaEngine, 1, func(i int, b *BlockGen) {
		b.SetCoinbase(common.Address{1})
	...
	expected := new(big.Int).Add(
		...
		silaash.ConstantinopleBlockReward.ToBig(),
	)
	...
	blocks, _ = GenerateChain(gspec.Config, block, silaEngine, genDb, 1, func(i int, b *BlockGen) {
		b.SetCoinbase(common.Address{2})
...
func testSetCanonical(t *testing.T, scheme string) {
	...
	// Generate and import the canonical chain
	_, canon, _ := GenerateChainWithGenesis(gspec, silaEngine, chainLength, func(i int, gen *BlockGen) {
	...
	options := DefaultConfig().WithStateScheme(scheme)
	chain, err := NewBlockChain(diskdb, gspec, silaEngine, options)
	if err != nil {
		t.Fatalf("failed to create tester chain: %v", err)
	}
	defer chain.Stop()
	...
	_, side, _ := GenerateChainWithGenesis(gspec, silaEngine, chainLength, func(i int, gen *BlockGen) {
...
func testCanonicalHashMarker(t *testing.T, scheme string) {
	...
	for _, c := range cases {
		...
		_, forkA, _ := GenerateChainWithGenesis(gspec, silaEngine, c.forkA, func(i int, gen *BlockGen) {})
		_, forkB, _ := GenerateChainWithGenesis(gspec, silaEngine, c.forkB, func(i int, gen *BlockGen) {})
...
func testCreateThenDelete(t *testing.T, config *params.ChainConfig) {
	...
	nonce := uint64(0)
	signer := types.HomesteadSigner{}
	_, blocks, _ := GenerateChainWithGenesis(gspec, silaEngine, 2, func(i int, b *BlockGen) {
		fee := big.NewInt(1)
...
func testChainReorgSnapSync(t *testing.T, ancientLimit uint64) {
	...
	chainA, receiptsA := GenerateChain(gspec.Config, blocks[len(blocks)-1], silaEngine, genDb, 16, func(i int, gen *BlockGen) {
		gen.SetCoinbase(common.Address{0: byte(0xa), 19: byte(i)})
	})
	chainB, receiptsB := GenerateChain(gspec.Config, blocks[len(blocks)-1], silaEngine, genDb, 20, func(i int, gen *BlockGen) {
	...
	chain, _ := NewBlockChain(db, gspec, beacon.New(silaash.NewFaker()), options)
	defer chain.Stop()
...
func testInsertChainWithCutoff(t *testing.T, cutoff uint64, ancientLimit uint64, genesis *Genesis, blocks []*types.Block, receipts []types.Receipts) {
	...
	chain, _ := NewBlockChain(db, genesis, beacon.New(silaash.NewFaker()), DefaultConfig().WithStateScheme(rawdb.PathScheme))
	defer chain.Stop()
...
func TestSetHeadBeyondRootFinalizedBug(t *testing.T) {
	...
	_, _, blockchain, err := newCanonical(silaash.NewFaker(), 100, true, rawdb.PathScheme)
	if err != nil {
		t.Fatalf("failed to create pristine chain: %v", err)
	}
	defer blockchain.Stop()
