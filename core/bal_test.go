...
func (e *balTestEnv) run(t *testing.T, gen func(*BlockGen)) (*bal.BlockAccessList, types.Receipts) {
	...
	_, blocks, receipts := GenerateChainWithGenesis(e.gspec, silaEngine, 1, func(_ int, b *BlockGen) {
		gen(b)
	})
...
// TestBALPrecompileValueTransferRecordsBalance: a precompile receives SILA only
