...
func testRepairWithScheme(t *testing.T, tt *rewindTest, snapshots bool, scheme string) {
	...
	if tt.sidechainBlocks > 0 {
		sideblocks, _ = GenerateChain(gspec.Config, gspec.ToBlock(), silaEngine, rawdb.NewMemoryDatabase(), tt.sidechainBlocks, func(i int, b *BlockGen) {
	...
	canonblocks, _ := GenerateChain(gspec.Config, gspec.ToBlock(), silaEngine, rawdb.NewMemoryDatabase(), tt.canonicalBlocks, func(i int, b *BlockGen) {
	...
	newChain, err := NewBlockChain(db, gspec, silaEngine, option)
	if err != nil {
...
func testIssue23496(t *testing.T, scheme string) {
	...
	chain, err = NewBlockChain(db, gspec, silaEngine, DefaultConfig().WithStateScheme(scheme))
	if err != nil {
		t.Fatalf("Failed to recreate chain: %v", err)
	}
	defer chain.Stop()
