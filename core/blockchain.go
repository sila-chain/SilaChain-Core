...
type BlockChain struct {
	...
	newPayloadFeed   event.Feed // Feed for silaSilaEngine API newPayload events
...
// and Processor.
func NewBlockChain(db siladb.Database, genesis *Genesis, silaEngine consensus.SilaEngine, cfg *BlockChainConfig) (*BlockChain, error) {
	if cfg == nil {
		cfg = DefaultConfig()
	}
	...
	bc.hc, err = NewHeaderChain(db, chainConfig, silaEngine, bc.insertStopped)
	if err != nil {
		return nil, err
	}
	...
	// the head block (silaash cache or clique voting snapshot). Might as well do
	// it in advance.
	bc.silaEngine.VerifyHeader(bc, bc.CurrentHeader())
...
func (bc *BlockChain) setHeadBeyondRoot(head uint64, time uint64, root common.Hash, repair bool) (uint64, error) {
	...
	updateFn := func(db siladb.KeyValueWriter, header *types.Header) (*types.Header, bool) {
	...
	delFn := func(db siladb.KeyValueWriter, hash common.Hash, num uint64) {
...
func (bc *BlockChain) writeBlockWithState(block *types.Block, receipts []*types.Receipt, statedb *state.StateDB) error {
	...
	if nodes > limit || imgs > 4*1024*1024 {
		bc.triedb.Cap(limit - siladb.IdealBatchSize)
	}
...
func (bc *BlockChain) insertChain(ctx context.Context, chain types.Blocks, setHead bool, makeWitness bool) (*stateless.Witness, int, error) {
	...
	abort, results := bc.silaEngine.VerifyHeaders(bc, headers)
	defer close(abort)
