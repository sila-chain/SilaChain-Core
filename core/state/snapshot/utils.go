...
func CheckDanglingStorage(chaindb siladb.KeyValueStore) error {
...
func checkDanglingDiskStorage(chaindb siladb.KeyValueStore) error {
	var (
		lastReport = time.Now()
		start      = time.Now()
		lastKey    []byte
...
// snapshot difflayers.
func checkDanglingMemStorage(db siladb.KeyValueStore) error {
	start := time.Now()
...
func CheckJournalAccount(db siladb.KeyValueStore, hash common.Hash) error {
