...
func newSilaPoWTestConfig(homestead *big.Int) *params.ChainConfig {
	...
	config.Ethash = &params.SilaPoWConfig{} // Retained for Sila PoW legacy JSON compatibility.
	return config
}
...
func testSetupGenesis(t *testing.T, scheme string) {
	var (
		...
		customg     = Genesis{
			Config: newSilaPoWTestConfig(big.NewInt(3)),
	...
	oldcustomg.Config = newSilaPoWTestConfig(big.NewInt(2))
	...
	tests := []struct {
		...
		{
			name: "sila default no block in DB, genesis == nil",
			fn: func(db ethdb.Database) (*params.ChainConfig, common.Hash, *params.ConfigCompatError, error) {
		...
		{
			name: "sila explicit genesis in empty DB",
			fn: func(db ethdb.Database) (*params.ChainConfig, common.Hash, *params.ConfigCompatError, error) {
				return SetupGenesisBlock(db, triedb.NewDatabase(db, newDbConfig(scheme)), SilaDefaultGenesisBlock())
			},
			wantHash:   params.SilaMainnetGenesisHash,
			wantConfig: params.SilaMainnetChainConfig,
		},
		{
			name: "sila block in DB, genesis == nil",
			fn: func(db ethdb.Database) (*params.ChainConfig, common.Hash, *params.ConfigCompatError, error) {
				SilaDefaultGenesisBlock().MustCommit(db, triedb.NewDatabase(db, newDbConfig(scheme)))
				return SetupGenesisBlock(db, triedb.NewDatabase(db, newDbConfig(scheme)), nil)
			},
			wantHash:   params.SilaMainnetGenesisHash,
			wantConfig: params.SilaMainnetChainConfig,
