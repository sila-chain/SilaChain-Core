...
func testSetupGenesis(t *testing.T, scheme string) {
	var (
		...
		customg     = Genesis{
			Config: &params.ChainConfig{HomesteadBlock: big.NewInt(3), Silaash: &params.SilaashConfig{}},
	...
	oldcustomg.Config = &params.ChainConfig{HomesteadBlock: big.NewInt(2), Silaash: &params.SilaashConfig{}}

	tests := []struct {
		name           string
		fn             func(siladb.Database) (*params.ChainConfig, common.Hash, *params.ConfigCompatError, error)
		...
		},
		{
			...
			fn: func(db siladb.Database) (*params.ChainConfig, common.Hash, *params.ConfigCompatError, error) {
				...
				bc, _ := NewBlockChain(db, &oldcustomg, silaash.NewFullFaker(), DefaultConfig().WithStateScheme(scheme))
				defer bc.Stop()

				_, blocks, _ := GenerateChainWithGenesis(&oldcustomg, silaash.NewFaker(), 4, nil)
