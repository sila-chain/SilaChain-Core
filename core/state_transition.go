...
type Message struct {
	...
	//
	// This field will be set to true for operations like RPC sila_call
...
//
// The balance requirement is the worst-case SILA the tx may need to lock
...
func (st *stateTransition) buyGas() error {
	...
	if st.evm.ChainConfig().IsCancun(st.evm.Context.BlockNumber, st.evm.Context.Time) {
		if blobGas := st.blobGasUsed(); blobGas > 0 {
			...
			// BlobGasFeeCap. However, in sila_call it is still possible for users to specify
...
// subsets of these checks for simulation paths (sila_call, sila_estimateGas).
func (st *stateTransition) preCheck() error {
	...
	if st.evm.ChainConfig().IsLondon(st.evm.Context.BlockNumber) {
		// Skip the checks if gas fields are zero and baseFee was explicitly disabled (sila_call)
	...
	if msg.BlobHashes != nil {
		...
		// However, messages created through RPC (sila_call) don't have this restriction.
		if msg.To == nil {
	...
	if st.evm.ChainConfig().IsCancun(st.evm.Context.BlockNumber, st.evm.Context.Time) {
		if st.blobGasUsed() > 0 {
			// Skip the checks if gas fields are zero and blobBaseFee was explicitly disabled (sila_call)
...
func (st *stateTransition) execute() (*ExecutionResult, error) {
	...
	// Settle down the gas usage and refund the SILA back if any remaining
...
//
//   - Refunds the leftover gas to the sender as SILA.
//
...
func (st *stateTransition) settleGas(rules params.Rules, floorDataGas uint64) (gasUsed, peakUsed uint64, err error) {
	...
	// Refund leftover gas to the sender as SILA.
	if gasLeft > 0 {
