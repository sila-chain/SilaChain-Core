...
type ChainContext interface {
	...
	// SilaEngine retrieves the chain's consensus silaEngine.
...
func NewEVMBlockContext(header *types.Header, chain ChainContext, author *common.Address) vm.BlockContext {
	...
	if author == nil {
		beneficiary, _ = chain.SilaEngine().Author(header) // Ignore error, we're past header validation
	} else {
		beneficiary = *author
	}
...
func Transfer(db vm.StateDB, sender, recipient common.Address, amount *uint256.Int, rules *params.Rules) {
	...
	if rules.IsAmsterdam && !amount.IsZero() && sender != recipient {
		db.AddLog(types.SilaTransferLog(sender, recipient, amount))
	}
}
