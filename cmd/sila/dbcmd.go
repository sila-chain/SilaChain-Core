...
func showDBStats(db siladb.KeyValueStater) {
	stats, err := db.Stat()
	if err != nil {
...
type snapshotIterator struct {
	init    bool
	account siladb.Iterator
	storage siladb.Iterator
}
...
var chainExporters = map[string]func(db siladb.Database) utils.ChainDataIterator{
	"preimage": func(db siladb.Database) utils.ChainDataIterator {
		iter := db.NewIterator(rawdb.PreimagePrefix, nil)
		return &preimageIterator{iter: iter}
	},
	"snapshot": func(db siladb.Database) utils.ChainDataIterator {
		account := db.NewIterator(rawdb.SnapshotAccountPrefix, nil)
		storage := db.NewIterator(rawdb.SnapshotStoragePrefix, nil)
		return &snapshotIterator{account: account, storage: storage}
	},
	"code": func(db siladb.Database) utils.ChainDataIterator {
