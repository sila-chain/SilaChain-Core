...
func TestCustomBackend(t *testing.T) {
	...
	for i, tt := range []backendTest{
		{ // When not specified, it should default to pebble
			execArgs:   []string{"--db.silaEngine", "pebble"},
			execExpect: "0x0000000000001338",
		},
		{ // Explicit leveldb
			initArgs:   []string{"--db.silaEngine", "leveldb"},
			execArgs:   []string{"--db.silaEngine", "leveldb"},
			execExpect: "0x0000000000001338",
		},
		...
		{ // Explicit pebble
			initArgs:   []string{"--db.silaEngine", "pebble"},
			execArgs:   []string{"--db.silaEngine", "pebble"},
			execExpect: "0x0000000000001338",
		},
		...
		{ // Can't start pebble on top of leveldb
			initArgs:   []string{"--db.silaEngine", "leveldb"},
			execArgs:   []string{"--db.silaEngine", "pebble"},
			execExpect: `Fatal: Failed to register the Sila service: db.silaEngine choice was pebble but found pre-existing leveldb database in specified data directory`,
		},
		{ // Can't start leveldb on top of pebble
			initArgs:   []string{"--db.silaEngine", "pebble"},
			execArgs:   []string{"--db.silaEngine", "leveldb"},
			execExpect: `Fatal: Failed to register the Sila service: db.silaEngine choice was leveldb but found pre-existing pebble database in specified data directory`,
		},
		{ // Reject invalid backend choice
			initArgs:   []string{"--db.silaEngine", "mssql"},
			initExpect: `Fatal: Invalid choice for db.silaEngine 'mssql', allowed 'leveldb' or 'pebble'`,
