...
// data.
func NewSuite(dest *enode.Node, chainDir, silaEngineURL, jwt string) (*Suite, error) {
	chain, err := NewChain(chainDir)
	if err != nil {
		return nil, err
	}
	silaEngine, err := NewSilaEngineClient(chainDir, silaEngineURL, jwt)
	if err != nil {
		return nil, err
	}
...
func (s *Suite) TestStatus(t *utesting.T) {
	t.Log(`This test is just a sanity check. It performs an sila protocol handshake.`)
