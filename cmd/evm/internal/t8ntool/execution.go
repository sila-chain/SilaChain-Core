...
func MakePreState(db siladb.Database, accounts types.GenesisAlloc, isBintrie bool) *state.StateDB {
...
func MakePreStateStreaming(db siladb.Database, allocPath string, isBintrie bool) (*state.StateDB, error) {
	tdb := triedb.NewDatabase(db, newPrestateTrieDBConfig(isBintrie))
	sdb := state.NewDatabase(tdb, nil)
	if isBintrie {
		sdb.(*state.UBTDatabase).EnableAllocRecording()
...
// calcDifficulty is based on silaash.CalcDifficulty. This method is used in case
// the caller does not provide an explicit difficulty, but instead provides only
// parent timestamp + difficulty.
// Note: this method only works for silaash silaEngine.
func calcDifficulty(config *params.ChainConfig, number, currentTime, parentTime uint64,
	...
	return silaash.CalcDifficulty(config, currentTime, parent)
}
