...
func SetNodeConfig(ctx *cli.Context, cfg *node.Config) {
	...
	if ctx.IsSet(DBEngineFlag.Name) {
		...
		if dbEngine != "leveldb" && dbEngine != "pebble" {
			Fatalf("Invalid choice for db.silaEngine '%s', allowed 'leveldb' or 'pebble'", dbEngine)
		}
		log.Info(fmt.Sprintf("Using %s as db silaEngine", dbEngine))
...
func RegisterSilaStatsService(stack *node.Node, backend *sila.SilaAPIBackend, url string) {
	if err := silastats.New(stack, backend, backend.SilaEngine(), url); err != nil {
		Fatalf("Failed to register the Sila Stats service: %v", err)
	}
}

// RegisterGraphQLService adds the GraphQL API to the node.
func RegisterGraphQLService(stack *node.Node, backend silaapi.Backend, filterSystem *filters.FilterSystem, cfg *node.Config) {
...
func RegisterFilterAPI(stack *node.Node, backend silaapi.Backend, ethcfg *silaconfig.Config) *filters.FilterSystem {
...
func RegisterSyncOverrideService(stack *node.Node, sila *sila.Sila, config syncer.Config) {
	...
	syncer.Register(stack, sila, config)
}
...
func MakeChainDatabase(ctx *cli.Context, stack *node.Node, readonly bool) siladb.Database {
	var (
...
func tryMakeReadOnlyDatabase(ctx *cli.Context, stack *node.Node) siladb.Database {
	// If the database doesn't exist we need to open it in write-mode to allow
	// the silaEngine to create files.
	readonly := true
...
func MakeChain(ctx *cli.Context, stack *node.Node, readonly bool) (*core.BlockChain, siladb.Database) {
	var (
	...
	silaEngine, err := silaconfig.CreateConsensusEngine(config, chainDb)
	if err != nil {
		Fatalf("%v", err)
	}
	if gcmode := ctx.String(GCModeFlag.Name); gcmode != "full" && gcmode != "archive" {
	...
	chain, err := core.NewBlockChain(chainDb, gspec, silaEngine, options)
	if err != nil {
...
func MakeTrieDatabase(ctx *cli.Context, stack *node.Node, disk siladb.Database, preimage bool, readOnly bool, isUBT bool) *triedb.Database {
