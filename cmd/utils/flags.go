...
var (
	...
	BlsyncApiFlag = &cli.StringFlag{
		...
		Usage:    "Target Sila Engine API URL",
		Category: flags.BeaconCategory,
	}
	BlsyncJWTSecretFlag = &flags.DirectoryFlag{
		Name:     "blsync.jwtsecret",
		Usage:    "Path to a JWT secret to use for target Sila Engine API endpoint",
		Category: flags.BeaconCategory,
	}
...
func setHTTP(ctx *cli.Context, cfg *node.Config) {
	...
	if ctx.Bool(LegacyEngineCompatibilityFlag.Name) {
		cfg.LegacyEngineCompatibility = true
	}
...
func SetP2PConfig(ctx *cli.Context, cfg *p2p.Config) {
	...
	log.Info("Maximum peer count", "Sila", ethPeers, "total", cfg.MaxPeers)
...
func RegisterFilterAPI(stack *node.Node, backend silabackend.Backend, ethcfg *ethconfig.Config) *filters.FilterSystem {
