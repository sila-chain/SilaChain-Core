...
func ImportPreimages(db siladb.Database, fn string) error {
...
// It's a part of the deprecated functionality, should be removed in the future.
func ExportPreimages(db siladb.Database, fn string) error {
...
func NewStateIterator(triedb *triedb.Database, db siladb.Database, root common.Hash) (*StateIterator, error) {
...
func ExportSnapshotPreimages(chaindb siladb.Database, stateIt *StateIterator, fn string, root common.Hash) error {
	log.Info("Exporting preimages", "file", fn)
...
type exportHeader struct {
	Magic    string // Always set to 'siladbdump' for disambiguation
	Version  uint64
	Kind     string
	UnixTime uint64
}

const exportMagic = "siladbdump"
const (
	OpBatchAdd = 0
	OpBatchDel = 1
)
...
func ImportLDBData(db siladb.Database, f string, startIndex int64, interrupt chan struct{}) error {
