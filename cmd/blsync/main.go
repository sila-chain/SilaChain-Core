...
func main() {
	...
	app.Flags = slices.Concat([]cli.Flag{
		...
		utils.MainnetFlag,
		utils.SilaPublicTestnetFlag,
		utils.SilaStagingTestnetFlag,
		utils.SilaDevTestnetFlag,
		utils.BlsyncApiFlag,
...
func makeRPCClient(ctx *cli.Context) *rpc.Client {
	...
	silaEngineApiUrl, jwtFileName := ctx.String(utils.BlsyncApiFlag.Name), ctx.String(utils.BlsyncJWTSecretFlag.Name)
	...
	cl, err := rpc.DialOptions(context.Background(), silaEngineApiUrl, rpc.WithHTTPAuth(auth))
	if err != nil {
