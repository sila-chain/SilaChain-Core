...
func NewDebugAPI(sila *Sila) *DebugAPI {
	return &DebugAPI{sila: sila}
}
...
func (api *DebugAPI) DumpBlock(blockNr rpc.BlockNumber) (state.Dump, error) {
	...
	if blockNr == rpc.PendingBlockNumber {
		...
		_, _, stateDb := api.sila.miner.Pending()
		if stateDb == nil {
	...
	case rpc.LatestBlockNumber:
		header = api.sila.blockchain.CurrentBlock()
	case rpc.FinalizedBlockNumber:
		header = api.sila.blockchain.CurrentFinalBlock()
	case rpc.SafeBlockNumber:
		header = api.sila.blockchain.CurrentSafeBlock()
	default:
		block := api.sila.blockchain.GetBlockByNumber(uint64(blockNr))
		if block == nil {
	...
	stateDb, err := api.sila.BlockChain().StateAt(header)
	if err != nil {
...
func (api *DebugAPI) Preimage(ctx context.Context, hash common.Hash) (hexutil.Bytes, error) {
	if preimage := rawdb.ReadPreimage(api.sila.ChainDb(), hash); preimage != nil {
		return preimage, nil
	}
...
func (api *DebugAPI) GetBadBlocks(ctx context.Context) ([]*BadBlockArgs, error) {
	var (
		blocks  = rawdb.ReadAllBadBlocks(api.sila.chainDb)
	...
	for _, block := range blocks {
		...
		blockJSON = silaapi.RPCMarshalBlock(block, true, true, api.sila.APIBackend.ChainConfig())
	...
	if number, ok := blockNrOrHash.Number(); ok {
		if number == rpc.PendingBlockNumber {
			...
			case rpc.SafeBlockNumber:
				header = api.sila.blockchain.CurrentSafeBlock()
			default:
				block := api.sila.blockchain.GetBlockByNumber(uint64(number))
				if block == nil {
		...
		stateDb, err = api.sila.BlockChain().StateAt(block.Header())
		if err != nil {
			return state.Dump{}, err
		}
	} else {
...
func (api *DebugAPI) StorageRangeAt(ctx context.Context, blockNrOrHash rpc.BlockNumberOrHash, txIndex int, contractAddress common.Address, keyStart hexutil.Bytes, maxResult int) (StorageRangeResult, error) {
	...
	block, err := api.sila.APIBackend.BlockByNumberOrHash(ctx, blockNrOrHash)
	if err != nil {
	...
	_, _, statedb, release, err := api.sila.stateAtTransaction(ctx, block, txIndex)
	if err != nil {
		return StorageRangeResult{}, err
	}
	defer release()
...
func (api *DebugAPI) GetModifiedAccountsByNumber(startNum uint64, endNum *uint64) ([]common.Address, error) {
	...
	startHeader = api.sila.blockchain.GetHeaderByNumber(startNum)
	if startHeader == nil {
	...
	if endNum == nil {
		...
		startHeader = api.sila.blockchain.GetHeaderByHash(startHeader.ParentHash)
		if startHeader == nil {
			return nil, fmt.Errorf("block %x has no parent", endHeader.Number)
		}
	} else {
		endHeader = api.sila.blockchain.GetHeaderByNumber(*endNum)
		if endHeader == nil {
...
func (api *DebugAPI) GetModifiedAccountsByHash(startHash common.Hash, endHash *common.Hash) ([]common.Address, error) {
	var startHeader, endHeader *types.Header
	startHeader = api.sila.blockchain.GetHeaderByHash(startHash)
	if startHeader == nil {
	...
	if endHash == nil {
		...
		endHeader = api.sila.blockchain.GetHeaderByHash(*endHash)
		if endHeader == nil {
...
func (api *DebugAPI) getModifiedAccounts(startHeader, endHeader *types.Header) ([]common.Address, error) {
	...
	triedb := api.sila.BlockChain().TrieDB()
...
func (api *DebugAPI) GetAccessibleState(from, to rpc.BlockNumber) (uint64, error) {
	if api.sila.blockchain.TrieDB().Scheme() == rawdb.PathScheme {
	...
	var resolveNum = func(num rpc.BlockNumber) (uint64, error) {
		...
		if num.Int64() < 0 {
			block := api.sila.blockchain.CurrentBlock()
			if block == nil {
	...
	for i := int64(start); i != int64(end); i += delta {
		...
		h := api.sila.BlockChain().GetHeaderByNumber(uint64(i))
		if h == nil {
			return 0, fmt.Errorf("missing header %d", i)
		}
		if ok, _ := api.sila.ChainDb().Has(h.Root[:]); ok {
			return uint64(i), nil
		}
	}
...
func (api *DebugAPI) SetTrieFlushInterval(interval string) error {
	...
	api.sila.blockchain.SetTrieFlushInterval(t)
	return nil
}
...
func (api *DebugAPI) GetTrieFlushInterval() (string, error) {
	...
	return api.sila.blockchain.GetTrieFlushInterval().String(), nil
}
...
func (api *DebugAPI) StateSize(blockHashOrNumber *rpc.BlockNumberOrHash) (interface{}, error) {
	sizer := api.sila.blockchain.StateSizer()
	if sizer == nil {
	...
	if blockHashOrNumber == nil {
		...
		header, herr := api.sila.APIBackend.HeaderByNumberOrHash(context.Background(), *blockHashOrNumber)
...
func (api *DebugAPI) ExecutionWitness(bn rpc.BlockNumberOrHash) (*stateless.ExtWitness, error) {
	bc := api.sila.blockchain
	block, err := api.sila.APIBackend.BlockByNumberOrHash(context.Background(), bn)
	if err != nil {
