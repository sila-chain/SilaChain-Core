...
func newTestHandler(mode silaconfig.SyncMode) *testHandler {
	return newTestHandlerWithBlocks(0, mode)
}

// newTestHandlerWithBlocks creates a new handler for testing purposes, with a
// given number of initial blocks.
func newTestHandlerWithBlocks(blocks int, mode silaconfig.SyncMode) *testHandler {
	...
	chain, _ := core.NewBlockChain(db, gspec, silaash.NewFaker(), nil)

	_, bs, _ := core.GenerateChainWithGenesis(gspec, silaash.NewFaker(), blocks, nil)
...
func TestBroadcastChoice(t *testing.T) {
	...
	var chosen49 = make([]map[*silaPeer]struct{}, len(txsenders))
...
func createTestPeers(rand *rand.Rand, n int) []*silaPeer {
	peers := make([]*silaPeer, n)
	for i := range peers {
		var id enode.ID
		rand.Read(id[:])
		p2pPeer := p2p.NewPeer(id, "test", nil)
		ep := sila.NewPeer(sila.SILA69, p2pPeer, nil, nil, nil)
		peers[i] = &silaPeer{Peer: ep}
	}
	return peers
}

func closePeers(peers []*silaPeer) {
	for _, p := range peers {
		p.Close()
	}
}
