...
func benchmarkFilters(b *testing.B, history uint64, noHistory bool) {
	...
	defer db.Close()
	_, chain, receipts := core.GenerateChainWithGenesis(gspec, silaash.NewFaker(), 100010, func(i int, gen *core.BlockGen) {
		switch i {
		case 2403:
...
func testFilters(t *testing.T, history uint64, noHistory bool) {
		...
		*/
		...
		gspec = &core.Genesis{
			...
			Alloc: types.GenesisAlloc{
				addr:      {Balance: big.NewInt(0).Mul(big.NewInt(100), big.NewInt(params.SilaBaseUnit))},
	...
	pchain, preceipts := core.GenerateChain(gspec.Config, chain[len(chain)-1], silaash.NewFaker(), db, 1, func(i int, gen *core.BlockGen) {
...
func TestRangeLimit(t *testing.T) {
	...
	chain, _ := core.GenerateChain(gspec.Config, gspec.ToBlock(), silaash.NewFaker(), db, 10, func(i int, gen *core.BlockGen) {})
	options := core.DefaultConfig().WithStateScheme(rawdb.HashScheme)
