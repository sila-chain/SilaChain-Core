...
// Progress is the set of snap-syncer progress that sila/downloader surfaces in
...
// state syncers, consumed by sila/downloader. Peers are passed as SyncPeerV2,
...
func NewV1Syncer(db siladb.Database, scheme string) Syncer {
	return syncerV1Adapter{newSyncer(db, scheme)}
}

// NewV2Syncer returns a Syncer backed by the snap/2 state syncer.
func NewV2Syncer(db siladb.Database, scheme string) Syncer {
