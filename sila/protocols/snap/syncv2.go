...
// snap protocol.
func newSyncerV2(db siladb.Database, scheme string) *syncerV2 {
	s := &syncerV2{
		db:     db,
		scheme: scheme,
...
func isPivotReorged(db siladb.Database, prev, curr *types.Header) bool {
...
func (s *syncerV2) loadSyncStatus() {
	...
	if raw := rawdb.ReadSnapshotSyncStatus(s.db); len(raw) > 0 {
		if raw[0] != syncProgressVersion {
			...
			// sila_syncing reports real stats during catch-up and trie generation
...
func deleteRange(batch siladb.Batch, prefix []byte) {
...
// sync data!
func deleteKeyRange(batch siladb.Batch, start, limit []byte) {
...
func (s *syncerV2) resetTrienodes(batch siladb.Batch) {
...
func (s *syncerV2) saveSyncStatusWithDB(db siladb.KeyValueWriter) {
