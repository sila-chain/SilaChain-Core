...
func newSyncer(db siladb.KeyValueStore, scheme string) *syncer {
	return &syncer{
		db:     db,
		scheme: scheme,
	...
	if status := rawdb.ReadSnapshotSyncStatus(s.db); status != nil {
		if err := json.Unmarshal(status, &progress); err != nil {
			...
			for _, task := range s.tasks {
				...
				task.genBatch = siladb.HookedBatch{
					Batch: s.db.NewBatch(),
				...
				for accountHash, subtasks := range task.SubTasks {
					for _, subtask := range subtasks {
						subtask.genBatch = siladb.HookedBatch{
							Batch: s.db.NewBatch(),
							OnPut: func(key []byte, value []byte) {
...
func (s *syncer) commitHealer(force bool) {
	if !force && s.healer.scheduler.MemSize() < siladb.IdealBatchSize {
		return
	}
	batch := s.db.NewBatch()
...
func (s *syncer) onHealState(paths [][]byte, value []byte) error {
	...
	if s.stateWriter.ValueSize() > siladb.IdealBatchSize {
