...
// enrEntry is the ENR entry which advertises `sila` protocol on the discovery.
...
// StartENRUpdater starts the `sila` ENR updater loop, which listens for chain
...
// currentENREntry constructs an `sila` ENR entry based on the current state of the chain.
