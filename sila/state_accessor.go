...
func (sila *Sila) hashState(ctx context.Context, block *types.Block, base *state.StateDB, readOnly bool, preferDisk bool) (statedb *state.StateDB, release tracers.StateReleaseFunc, err error) {
	var (
		current  *types.Block
	...
	if readOnly {
		...
		if statedb, err = sila.blockchain.StateAt(block.Header()); err == nil {
			sila.blockchain.TrieDB().Reference(block.Root(), common.Hash{})
			return statedb, func() {
				sila.blockchain.TrieDB().Dereference(block.Root())
			}, nil
		}
	}
	...
	if base != nil {
		if preferDisk {
			...
			tdb := triedb.NewDatabase(sila.chainDb, triedb.HashDefaults)
		...
		current = sila.blockchain.GetBlock(block.ParentHash(), block.NumberU64()-1)
	} else {
		...
		for i := uint64(0); i < reexecLimit; i++ {
			...
			parent := sila.blockchain.GetBlock(current.ParentHash(), current.NumberU64()-1)
			if parent == nil {
	...
	for current.NumberU64() < origin {
		...
		if current = sila.blockchain.GetBlockByNumber(next); current == nil {
			return nil, nil, fmt.Errorf("block #%d not found", next)
		}
		_, err := sila.blockchain.Processor().Process(ctx, current, statedb, nil, vm.Config{})
		if err != nil {
			return nil, nil, fmt.Errorf("processing block %d failed: %v", current.NumberU64(), err)
		}
		// Finalize the state so any modifications are written to the trie
		root, err := statedb.Commit(current.NumberU64(), sila.blockchain.Config().IsEIP158(current.Number()), sila.blockchain.Config().IsCancun(current.Number(), current.Time()))
		if err != nil {
...
func (sila *Sila) pathState(block *types.Block) (*state.StateDB, func(), error) {
	// Check if the requested state is available in the live chain.
	header := block.Header()
	statedb, err := sila.blockchain.StateAt(header)
	if err == nil {
		return statedb, noopReleaser, nil
	}
	statedb, err = sila.blockchain.HistoricState(header)
	if err == nil {
		return statedb, noopReleaser, nil
	}
...
//     on disk.
func (sila *Sila) stateAtBlock(ctx context.Context, block *types.Block, base *state.StateDB, readOnly bool, preferDisk bool) (statedb *state.StateDB, release tracers.StateReleaseFunc, err error) {
	if sila.blockchain.TrieDB().Scheme() == rawdb.HashScheme {
		return sila.hashState(ctx, block, base, readOnly, preferDisk)
	}
	return sila.pathState(block)
}
...
func (sila *Sila) stateAtTransaction(ctx context.Context, block *types.Block, txIndex int) (*types.Transaction, vm.BlockContext, *state.StateDB, tracers.StateReleaseFunc, error) {
	// Short circuit if it's genesis block.
	if block.NumberU64() == 0 {
		return nil, vm.BlockContext{}, nil, nil, errors.New("no transaction in genesis")
	}
	// Create the parent state database
	parent := sila.blockchain.GetBlock(block.ParentHash(), block.NumberU64()-1)
	if parent == nil {
		return nil, vm.BlockContext{}, nil, nil, fmt.Errorf("parent %#x not found", block.ParentHash())
	}
	// Lookup the statedb of parent block from the live database,
	// otherwise regenerate it on the flight.
	statedb, release, err := sila.stateAtBlock(ctx, parent, nil, true, false)
	if err != nil {
	...
	core.PreExecution(ctx, block.BeaconRoot(), block.ParentHash(), sila.blockchain.Config(), evm, block.Number(), block.Time())

	if txIndex == 0 && len(block.Transactions()) == 0 {
		return nil, context, statedb, release, nil
	}
	// Recompute transactions up to the target index.
	signer := types.MakeSigner(sila.blockchain.Config(), block.Number(), block.Time())
