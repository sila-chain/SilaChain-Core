...
type peerConnection struct {
	...
	version uint       // Sila protocol version number to switch strategies
...
type Peer interface {
	RequestHeadersByHash(common.Hash, int, int, bool, chan *sila.Response) (*sila.Request, error)
	RequestHeadersByNumber(uint64, int, int, bool, chan *sila.Response) (*sila.Request, error)

	RequestBodies([]common.Hash, chan *sila.Response) (*sila.Request, error)
	RequestReceipts([]common.Hash, []uint64, []uint64, chan *sila.Response) (*sila.Request, error)
}
...
func (p *peerConnection) UpdateHeaderRate(delivered int, elapsed time.Duration) {
	p.rates.Update(sila.BlockHeadersMsg, elapsed, delivered)
}
...
func (p *peerConnection) UpdateBodyRate(delivered int, elapsed time.Duration) {
	p.rates.Update(sila.BlockBodiesMsg, elapsed, delivered)
}
...
func (p *peerConnection) UpdateReceiptRate(delivered int, elapsed time.Duration) {
	p.rates.Update(sila.ReceiptsMsg, elapsed, delivered)
}

// HeaderCapacity retrieves the peer's header download allowance based on its
// previously discovered throughput.
func (p *peerConnection) HeaderCapacity(targetRTT time.Duration) int {
	cap := p.rates.Capacity(sila.BlockHeadersMsg, targetRTT)
...
func (p *peerConnection) BodyCapacity(targetRTT time.Duration) int {
	cap := p.rates.Capacity(sila.BlockBodiesMsg, targetRTT)
...
func (p *peerConnection) ReceiptCapacity(targetRTT time.Duration) int {
	cap := p.rates.Capacity(sila.ReceiptsMsg, targetRTT)
...
func newPeerSet() *peerSet {
	return &peerSet{
		...
		rates: msgrate.NewTrackers(log.New("proto", "sila")),
	}
}
