...
type syncModer struct {
	mode  silaconfig.SyncMode
	chain BlockChain
	disk  siladb.KeyValueReader
	lock  sync.Mutex
}

func newSyncModer(mode silaconfig.SyncMode, chain BlockChain, disk siladb.KeyValueReader) *syncModer {
	if mode == silaconfig.FullSync {
...
func (m *syncModer) get(report bool) silaconfig.SyncMode {
	m.lock.Lock()
	defer m.lock.Unlock()

	// If we're in snap sync mode, return that directly
	if m.mode == silaconfig.SnapSync {
		return silaconfig.SnapSync
	}
	logger := log.Debug
	if report {
		logger = log.Info
