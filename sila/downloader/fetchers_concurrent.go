...
type typedQueue interface {
	...
	request(peer *peerConnection, req *fetchRequest, resCh chan *sila.Response) (*sila.Request, error)
	...
	deliver(peer *peerConnection, packet *sila.Response) (int, error)
}
...
func (d *Downloader) concurrentFetch(queue typedQueue) error {
	...
	responses := make(chan *sila.Response)

	// Track the currently active requests and their timeout order
	pending := make(map[string]*sila.Request)
	defer func() {
	...
	ordering := make(map[*sila.Request]int)
	timeouts := prque.New[int64, *sila.Request](func(data *sila.Request, index int) {
		ordering[data] = index
	})
