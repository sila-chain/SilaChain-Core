...
var (
	...
	maxQueuedHeaders           = 32 * 1024                        // [sila/62] Maximum number of headers to queue for import (DOS protection)
...
// Deprecated: use silaconfig.SyncMode instead
type SyncMode = silaconfig.SyncMode

const (
	// Deprecated: use silaconfig.FullSync
	FullSync = silaconfig.FullSync
	// Deprecated: use silaconfig.SnapSync
	SnapSync = silaconfig.SnapSync
)
...
type Downloader struct {
	...
	stateDB siladb.Database // Database to state sync into (and deduplicate via)
	...
	// Skeleton sync
	skeleton *skeleton // Header skeleton to backfill the chain with (sila mode)
...
func New(stateDb siladb.Database, mode silaconfig.SyncMode, chain BlockChain, dropPeer peerDropFn, success func(), snapV2 bool) *Downloader {
...
func (d *Downloader) Progress() sila.SyncProgress {
	...
	mode := d.getMode()
	switch mode {
	case silaconfig.FullSync:
		current = d.blockchain.CurrentBlock().Number.Uint64()
	case silaconfig.SnapSync:
...
func (d *Downloader) synchronise(beaconPing chan struct{}) (err error) {
	...
	defer func() {
		if err == nil && mode == silaconfig.SnapSync {
			d.moder.disableSnap()
...
func (d *Downloader) syncToHead() (err error) {
	...
	if mode == silaconfig.SnapSync && pivot == nil {
	...
	if mode == silaconfig.SnapSync && pivot.Number.Uint64() != 0 {
	...
	if mode == silaconfig.SnapSync && d.chainCutoffNumber != 0 {
	...
	} else if mode == silaconfig.FullSync {
	...
	for {
		...
		case task := <-d.headerProcCh:
			...
			for len(headers) > 0 {
				...
				if mode == silaconfig.SnapSync && cutoff != 0 {
...
func (d *Downloader) importBlockResults(results []*fetchResult) error {
	...
	if index, err := d.blockchain.InsertChain(blocks); err != nil {
		if index < len(results) {
			...
			// In post-merge, notify the silaSilaEngine API of encountered bad chains
			if d.badBlock != nil {
