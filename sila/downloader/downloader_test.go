...
func newTester(t *testing.T, mode silaconfig.SyncMode) *downloadTester {
	return newTesterWithNotification(t, mode, nil)
}

// newTesterWithNotification creates a new downloader test mocker (snap/1).
func newTesterWithNotification(t *testing.T, mode silaconfig.SyncMode, success func()) *downloadTester {
...
func newTesterWithSnap(t *testing.T, mode silaconfig.SyncMode, success func(), snapV2 bool) *downloadTester {
	...
	chain, err := core.NewBlockChain(db, gspec, silaash.NewFaker(), nil)
	if err != nil {
		panic(err)
	}
