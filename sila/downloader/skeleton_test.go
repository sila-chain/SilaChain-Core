...
func (p *skeletonTestPeer) RequestHeadersByNumber(origin uint64, amount int, skip int, reverse bool, sink chan *sila.Response) (*sila.Request, error) {
	...
	if headers == nil {
		...
		if len(p.headers) > int(origin) { // Don't serve headers if we're missing the origin
			for i := 0; i < amount; i++ {
				...
				// attack by sending/receiving those over sila.
...
func (p *skeletonTestPeer) RequestHeadersByHash(common.Hash, int, int, bool, chan *sila.Response) (*sila.Request, error) {
	panic("skeleton sync must not request headers by hash")
}

func (p *skeletonTestPeer) RequestBodies([]common.Hash, chan *sila.Response) (*sila.Request, error) {
	panic("skeleton sync must not request block bodies")
}

func (p *skeletonTestPeer) RequestReceipts([]common.Hash, []uint64, []uint64, chan *sila.Response) (*sila.Request, error) {
...
func TestSkeletonSyncRetrievals(t *testing.T) {
	...
	for i, tt := range tests {
		...
		for _, peer := range tt.peers {
			peerset.Register(newPeerConnection(peer.id, sila.SILA69, peer, log.New("id", peer.id)))
		}
		...
		endpeers := tt.peers
		if tt.newPeer != nil {
			if err := peerset.Register(newPeerConnection(tt.newPeer.id, sila.SILA69, tt.newPeer, log.New("id", tt.newPeer.id))); err != nil {
...
func TestSkeletonLinkSkipsNonCanonical(t *testing.T) {
	...
	if err := peerset.Register(newPeerConnection(peer.id, sila.SILA69, peer, log.New("id", peer.id))); err != nil {
...
func checkSkeletonProgress(db siladb.KeyValueReader, unpredictable bool, peers []*skeletonTestPeer, expected skeletonExpect) error {
	var progress skeletonProgress
