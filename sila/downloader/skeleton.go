...
func newSkeleton(db siladb.Database, peers *peerSet, drop peerDropFn, filler backfiller, chain chainReader) *skeleton {
	sk := &skeleton{
		db:         db,
		filler:     filler,
		chain:      chain,
...
func (s *skeleton) saveSyncStatus(db siladb.KeyValueWriter) {
...
func (s *skeleton) assignTasks(success chan *headerResponse, fail chan *headerRequest, cancel chan struct{}) {
	...
	for _, peer := range s.idles {
		...
		idlers.caps = append(idlers.caps, s.peers.rates.Capacity(peer.id, sila.BlockHeadersMsg, targetTTL))
	}
...
func (s *skeleton) executeTask(peer *peerConnection, req *headerRequest) {
	...
	case <-timeoutTimer.C:
		...
		s.peers.rates.Update(peer.id, sila.BlockHeadersMsg, 0, 0)
		s.scheduleRevertRequest(req)
	...
	case res := <-resCh:
		...
		headers := *res.Res.(*sila.BlockHeadersRequest)

		headerReqTimer.Update(time.Since(start))
		s.peers.rates.Update(peer.id, sila.BlockHeadersMsg, res.Time, len(headers))
...
func (s *skeleton) processResponse(res *headerResponse) (linked bool, merged bool) {
	...
	for s.scratchSpace[0] != nil {
		...
		if linked {
			...
			// the silaSilaEngine API, we will not push the subchains forward. This will
