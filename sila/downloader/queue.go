...
func (q *queue) Schedule(headers []*types.Header, hashes []common.Hash, from uint64) int {
	...
	for i, header := range headers {
		...
		if q.mode == silaconfig.SnapSync && !header.EmptyReceipts() {
...
func (q *queue) reserveHeaders(p *peerConnection, count int, taskPool map[common.Hash]*types.Header, taskQueue *prque.Prque[int64, *types.Header],
	...
	for len(send) < count && !taskQueue.Empty() {
		...
		stale, throttle, item, err := q.resultCache.AddFetch(header, q.mode == silaconfig.SnapSync)
		if stale {
...
func (q *queue) DeliverBodies(id string, hashes sila.BlockBodyHashes, bodies []sila.BlockBody) (int, error) {
	q.lock.Lock()
	defer q.lock.Unlock()
