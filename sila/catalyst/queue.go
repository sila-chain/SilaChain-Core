...
// silaEngine tracks before evicting old ones. Ideally we should only ever track the
// latest one; but have a slight wiggle room for non-ideal conditions.
const maxTrackedPayloads = 10

// maxTrackedHeaders is the maximum number of executed payloads the execution
// silaEngine tracks before evicting old ones. These are tracked outside the chain
...
func (q *payloadQueue) put(id silaEngine.PayloadID, payload *miner.Payload) {
	q.lock.Lock()
	defer q.lock.Unlock()
...
func (q *payloadQueue) has(id silaEngine.PayloadID) bool {
	q.lock.RLock()
	defer q.lock.RUnlock()
