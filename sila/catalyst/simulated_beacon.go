...
func payloadVersion(config *params.ChainConfig, time uint64) silaEngine.PayloadVersion {
	switch config.LatestFork(time) {
	case forks.Amsterdam:
		return silaEngine.PayloadV4
	case forks.BPO5, forks.BPO4, forks.BPO3, forks.BPO2, forks.BPO1, forks.Osaka, forks.Prague, forks.Cancun:
		return silaEngine.PayloadV3
	case forks.Paris, forks.Shanghai:
		return silaEngine.PayloadV2
	}
	panic("invalid fork, simulated beacon needs to be started post-merge")
}

// NewSimulatedBeacon constructs a new simulated beacon chain.
func NewSimulatedBeacon(period uint64, feeRecipient common.Address, sila *sila.Sila) (*SimulatedBeacon, error) {
	block := sila.BlockChain().CurrentBlock()
	current := silaEngine.ForkchoiceStateV1{
...
func (c *SimulatedBeacon) sealBlock(withdrawals []*types.Withdrawal, timestamp uint64) error {
	...
	if header := c.sila.BlockChain().CurrentBlock(); c.curForkchoiceState.HeadBlockHash != header.Hash() {
	...
	if err := c.sila.APIBackend.TxPool().Sync(); err != nil {
		return fmt.Errorf("failed to sync txpool: %w", err)
	}

	version := payloadVersion(c.sila.BlockChain().Config(), timestamp)
	tracer := otel.Tracer("")
	...
	if c.sila.BlockChain().Config().LatestFork(timestamp) == forks.Amsterdam {
		slotNumber := uint64(0)
	...
	if fcResponse == silaEngine.STATUS_SYNCING {
	...
	if fcResponse.PayloadStatus.Status == silaEngine.VALID && fcResponse.PayloadID == nil {
		return nil
	}
	...
	if version > silaEngine.PayloadV2 {
...
func (c *SimulatedBeacon) finalizedBlockHash(number uint64) *common.Hash {
	...
	if finalizedBlock := c.sila.BlockChain().GetBlockByNumber(finalizedNumber); finalizedBlock != nil {
...
func (c *SimulatedBeacon) setCurrentState(headHash, finalizedHash common.Hash) {
	c.curForkchoiceState = silaEngine.ForkchoiceStateV1{
...
func (c *SimulatedBeacon) Commit() common.Hash {
	...
	return c.sila.BlockChain().CurrentBlock().Hash()
}
...
func (c *SimulatedBeacon) Fork(parentHash common.Hash) error {
	...
	c.sila.TxPool().Sync()
	if pending, _ := c.sila.TxPool().Pending(txpool.PendingFilter{}); len(pending) != 0 {
		return errors.New("pending block dirty")
	}

	parent := c.sila.BlockChain().GetBlockByHash(parentHash)
	if parent == nil {
		return errors.New("parent not found")
	}
	_, err := c.sila.BlockChain().SetCanonical(parent)
	return err
}
...
func (c *SimulatedBeacon) AdjustTime(adjustment time.Duration) error {
	...
	parent := c.sila.BlockChain().CurrentBlock()
	if parent == nil {
		return errors.New("parent not found")
	}
	withdrawals := c.withdrawals.pop(10)
