...
func (api *ConsensusAPI) ExecuteStatelessPayloadV1(params silaEngine.ExecutableData, opaqueWitness hexutil.Bytes) (silaEngine.StatelessPayloadStatusV1, error) {
	if params.Withdrawals != nil {
		return silaEngine.StatelessPayloadStatusV1{Status: silaEngine.INVALID}, silaEngine.InvalidParams.With(errors.New("withdrawals not supported in V1"))
	}
...
func (api *ConsensusAPI) ExecuteStatelessPayloadV2(params silaEngine.ExecutableData, opaqueWitness hexutil.Bytes) (silaEngine.StatelessPayloadStatusV1, error) {
	...
	switch {
	case cancun:
		return silaEngine.StatelessPayloadStatusV1{Status: silaEngine.INVALID}, paramsErr("can't use newPayloadV2 post-cancun")
	case shanghai && params.Withdrawals == nil:
		return silaEngine.StatelessPayloadStatusV1{Status: silaEngine.INVALID}, paramsErr("nil withdrawals post-shanghai")
	case !shanghai && params.Withdrawals != nil:
		return silaEngine.StatelessPayloadStatusV1{Status: silaEngine.INVALID}, paramsErr("non-nil withdrawals pre-shanghai")
	case params.ExcessBlobGas != nil:
		return silaEngine.StatelessPayloadStatusV1{Status: silaEngine.INVALID}, paramsErr("non-nil excessBlobGas pre-cancun")
	case params.BlobGasUsed != nil:
		return silaEngine.StatelessPayloadStatusV1{Status: silaEngine.INVALID}, paramsErr("non-nil blobGasUsed pre-cancun")
	}
	return api.executeStatelessPayload(params, nil, nil, nil, opaqueWitness)
}
...
func (api *ConsensusAPI) ExecuteStatelessPayloadV3(params silaEngine.ExecutableData, versionedHashes []common.Hash, beaconRoot *common.Hash, opaqueWitness hexutil.Bytes) (silaEngine.StatelessPayloadStatusV1, error) {
	...
	case params.Withdrawals == nil:
		return silaEngine.StatelessPayloadStatusV1{Status: silaEngine.INVALID}, paramsErr("nil withdrawals post-shanghai")
	case params.ExcessBlobGas == nil:
		return silaEngine.StatelessPayloadStatusV1{Status: silaEngine.INVALID}, paramsErr("nil excessBlobGas post-cancun")
	case params.BlobGasUsed == nil:
		return silaEngine.StatelessPayloadStatusV1{Status: silaEngine.INVALID}, paramsErr("nil blobGasUsed post-cancun")
	case versionedHashes == nil:
		return silaEngine.StatelessPayloadStatusV1{Status: silaEngine.INVALID}, paramsErr("nil versionedHashes post-cancun")
	case beaconRoot == nil:
		return silaEngine.StatelessPayloadStatusV1{Status: silaEngine.INVALID}, paramsErr("nil beaconRoot post-cancun")
	case !api.checkFork(params.Timestamp, forks.Cancun):
		return silaEngine.StatelessPayloadStatusV1{Status: silaEngine.INVALID}, unsupportedForkErr("newPayloadV3 must only be called for cancun payloads")
	}
...
func (api *ConsensusAPI) ExecuteStatelessPayloadV4(params silaEngine.ExecutableData, versionedHashes []common.Hash, beaconRoot *common.Hash, executionRequests []hexutil.Bytes, opaqueWitness hexutil.Bytes) (silaEngine.StatelessPayloadStatusV1, error) {
	...
	case beaconRoot == nil:
		return silaEngine.StatelessPayloadStatusV1{Status: silaEngine.INVALID}, paramsErr("nil beaconRoot post-cancun")
	case executionRequests == nil:
		return silaEngine.StatelessPayloadStatusV1{Status: silaEngine.INVALID}, paramsErr("nil executionRequests post-prague")
	case !api.checkFork(params.Timestamp, forks.Prague, forks.Osaka, forks.BPO1, forks.BPO2, forks.BPO3, forks.BPO4, forks.BPO5):
		return silaEngine.StatelessPayloadStatusV1{Status: silaEngine.INVALID}, unsupportedForkErr("newPayloadV4 must only be called for prague/osaka payloads")
	}
	requests := convertRequests(executionRequests)
	if err := validateRequests(requests); err != nil {
		return silaEngine.StatelessPayloadStatusV1{Status: silaEngine.INVALID}, silaEngine.InvalidParams.With(err)
	}
	return api.executeStatelessPayload(params, versionedHashes, beaconRoot, requests, opaqueWitness)
}

func (api *ConsensusAPI) executeStatelessPayload(params silaEngine.ExecutableData, versionedHashes []common.Hash, beaconRoot *common.Hash, requests [][]byte, opaqueWitness hexutil.Bytes) (silaEngine.StatelessPayloadStatusV1, error) {
	log.Trace("SilaEngine API request received", "method", "ExecuteStatelessPayload", "number", params.Number, "hash", params.BlockHash)
	block, err := silaEngine.ExecutableDataToBlockNoHash(params, versionedHashes, beaconRoot, requests)
	if err != nil {
		bgu := "nil"
	...
	return silaEngine.StatelessPayloadStatusV1{Status: silaEngine.VALID, StateRoot: stateRoot, ReceiptsRoot: receiptRoot}, nil
}
