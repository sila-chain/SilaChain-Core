...
func startSimulatedBeaconEthService(t *testing.T, genesis *core.Genesis, period uint64) (*node.Node, *sila.Sila, *SimulatedBeacon) {
	...
	ethcfg := &silaconfig.Config{Genesis: genesis, SyncMode: silaconfig.FullSync, TrieTimeout: time.Minute, TrieDirtyCache: 256, TrieCleanCache: 256, Miner: miner.DefaultConfig}
...
func TestSimulatedBeaconSendWithdrawals(t *testing.T) {
	...
	node, silaService, mock := startSimulatedBeaconEthService(t, genesis, 1)
	_ = mock
	defer node.Close()

	chainHeadCh := make(chan core.ChainHeadEvent, 10)
	subscription := silaService.BlockChain().SubscribeChainHeadEvent(chainHeadCh)
	...
	for i := 0; i < 20; i++ {
		...
		if err := silaService.APIBackend.SendTx(context.Background(), tx); err != nil {
	...
	for {
		select {
		case ev := <-chainHeadCh:
			block := silaService.BlockChain().GetBlock(ev.Header.Hash(), ev.Header.Number.Uint64())
...
func TestOnDemandSpam(t *testing.T) {
	...
	go func() {
		for i := 0; i < txCount; i++ {
			...
			if err := sila.TxPool().Add([]*types.Transaction{tx}, false)[0]; err != nil {
	...
	for {
		select {
		case ev := <-chainHeadCh:
			block := sila.BlockChain().GetBlock(ev.Header.Hash(), ev.Header.Number.Uint64())
