...
func generateMergeChain(n int, merged bool) (*core.Genesis, []*types.Block) {
	...
	_, blocks, _ := core.GenerateChainWithGenesis(genesis, silaEngine, n, generate)
...
func TestSilaNewBlock(t *testing.T) {
	...
	for i := 0; i < 10; i++ {
		...
		block, err := silaEngine.ExecutableDataToBlock(*execData, nil, nil, nil)
		if err != nil {
...
func startEthService(t testing.TB, genesis *core.Genesis, blocks []*types.Block) (*node.Node, *sila.Sila) {
	t.Helper()
	...
	ethcfg := &silaconfig.Config{
		Genesis:        genesis,
		SyncMode:       silaconfig.FullSync,
		TrieTimeout:    time.Minute,
		TrieDirtyCache: 256,
		TrieCleanCache: 256,
		Miner:          miner.DefaultConfig,
	}
	ethservice, err := sila.New(n, ethcfg)
	if err != nil {
		t.Fatal("can't create sila service:", err)
	}
...
func setupBlocks(t *testing.T, ethservice *sila.Sila, n int, parent *types.Header, callback func(parent *types.Header), withdrawals [][]*types.Withdrawal, beaconRoots []common.Hash) []*types.Header {
	api := newConsensusAPIWithoutHeartbeat(ethservice)
...
func TestExchangeTransitionConfig(t *testing.T) {
	...
	// invalid ttd
	api := newConsensusAPIWithoutHeartbeat(ethservice)
	config := silaEngine.TransitionConfigurationV1{
...
func assembleEnvelope(api *ConsensusAPI, parentHash common.Hash, params *silaEngine.SilaPayloadAttributes) (*silaEngine.SilaExecutionPayloadEnvelope, error) {
	...
	payload, err := api.sila.Miner().BuildPayload(context.Background(), args, false)
	if err != nil {
		return nil, err
	}
...
func TestEmptyBlocks(t *testing.T) {
	...
	if status.Status != silaEngine.VALID {
	...
	if status.Status != silaEngine.INVALID {
	...
	if status.Status != silaEngine.SYNCING {
...
func setBlockhash(data *silaEngine.ExecutableData) *silaEngine.ExecutableData {
...
func TestWithdrawals(t *testing.T) {
	...
	if resp.PayloadStatus.Status != silaEngine.VALID {
		t.Fatalf("unexpected status (got: %s, want: %s)", resp.PayloadStatus.Status, silaEngine.VALID)
	}
...
func TestNilWithdrawals(t *testing.T) {
	...
	for _, test := range tests {
		var (
			err            error
			payloadVersion silaEngine.PayloadVersion
...
func setupBodies(t *testing.T) (*node.Node, *sila.Sila, []*types.Block) {
	genesis, blocks := generateMergeChain(10, true)
...
func TestGetBlockBodiesByHash(t *testing.T) {
	node, sila, blocks := setupBodies(t)
	...
	tests := []struct {
		...
		// First pow block
		{
			results: []*types.Body{sila.BlockChain().GetBlockByNumber(0).Body()},
			hashes:  []common.Hash{sila.BlockChain().GetBlockByNumber(0).Hash()},
		},
		// Last pow block
		{
...
func TestGetBlockBodiesByRangeInvalidParams(t *testing.T) {
	node, sila, _ := setupBodies(t)
	api := newConsensusAPIWithoutHeartbeat(sila)
	defer node.Close()
	tests := []struct {
		start hexutil.Uint64
		count hexutil.Uint64
		want  *silaEngine.SilaEngineAPIError
	}{
		// Genesis
		{
			start: 0,
			count: 1,
			want:  silaEngine.InvalidParams,
		},
		// No block requested
		{
			start: 1,
		...
		// More than 1024 blocks
		{
			start: 1,
			count: 1025,
			want:  silaEngine.TooLargeRequest,
		},
	}
...
func TestBlockToPayloadWithBlobs(t *testing.T) {
	...
	envelope := silaEngine.BlockToExecutableData(block, nil, sidecars, nil)
	var want int
	for _, tx := range txs {
...
func TestGetClientVersion(t *testing.T) {
	...
	api := newConsensusAPIWithoutHeartbeat(ethservice)
	info := silaEngine.ClientVersionV1{
		Code:    "TT",
		Name:    "test",
		Version: "1.1.1",
		Commit:  "0x12345678",
	...
	info = infos[0]
	if info.Code != silaEngine.ClientCode || info.Name != silaEngine.ClientName || info.Version != version.WithMeta {
...
func newGetBlobEnv(t testing.TB, version byte) (*node.Node, *ConsensusAPI) {
	...
	n, silaServ := startEthService(t, gspec, nil)
	...
	silaServ.TxPool().Add([]*types.Transaction{tx1, tx2, tx3}, true)
...
type getBlobsFn func(ctx context.Context, hashes []common.Hash) (silaEngine.BlobAndProofListV2, error)
...
func runGetBlobs(t testing.TB, getBlobs getBlobsFn, start, limit int, fillRandom bool, expectPartialResponse bool, name string) {
	...
	var (
		vhashes []common.Hash
		expect  silaEngine.BlobAndProofListV2
	)
	for j := start; j < limit; j++ {
		...
		expect = append(expect, &silaEngine.BlobAndProofV2{
			Blob:       testBlobs[j][:],
