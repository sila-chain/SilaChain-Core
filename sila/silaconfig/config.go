...
// Package silaconfig contains the configuration of the SILA and LES protocols.
package silaconfig
...
// Config contains configuration options for SILA and LES protocols.
type Config struct {
	...
	// RPCGasCap is the global gas cap for sila-call variants.
	RPCGasCap uint64

	// RPCEVMTimeout is the global timeout for sila-call.
...
// CreateConsensusEngine creates a consensus silaEngine for the given chain config.
// Clique is allowed for now to live standalone, but silaash is forbidden and can
// only exist on already merged networks.
func CreateConsensusEngine(config *params.ChainConfig, db siladb.Database) (consensus.SilaEngine, error) {
	...
	return beacon.New(silaash.NewFaker()), nil
}
