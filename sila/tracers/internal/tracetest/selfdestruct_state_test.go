...
func setupTestBlockchain(t *testing.T, genesis *core.Genesis, tx *types.Transaction, useBeacon bool) (*core.BlockChain, *types.Block, *state.StateDB) {
	var silaEngine consensus.SilaEngine
	if useBeacon {
	...
	_, blocks, _ := core.GenerateChainWithGenesis(genesis, silaEngine, 1, func(i int, b *core.BlockGen) {
		b.AddTx(tx)
	})
	db := rawdb.NewMemoryDatabase()
	blockchain, err := core.NewBlockChain(db, genesis, silaEngine, nil)
	if err != nil {
