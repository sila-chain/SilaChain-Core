...
import (
	...
	"github.com/sila-org/sila/sila/fetcher"
...
type handlerConfig struct {
	...
	Database       siladb.Database         // Database for direct sync insertions
	Chain          *core.BlockChain       // Blockchain to serve data from
	TxPool         txPool                 // Transaction pool to propagate from
	Network        uint64                 // Network identifier to advertise
	Sync           silaconfig.SyncMode     // Whether to snap or full sync
...
func newHandler(config *handlerConfig) (*handler, error) {
	...
	if h.downloader.ConfigSyncMode() == silaconfig.SnapSync && (config.Chain.Snapshots() == nil && config.Chain.TrieDB().Scheme() == rawdb.HashScheme) {
...
// runEthPeer registers an sila peer into the joint sila/snap peerset, adds it to
// various subsystems and starts handling messages.
func (h *handler) runEthPeer(peer *sila.Peer, handler sila.Handler) error {
	if !h.incHandlers() {
	...
	if h.downloader.ConfigSyncMode() == silaconfig.SnapSync {
	...
	if err := h.downloader.RegisterPeer(peer.ID(), peer.Version(), peer); err != nil {
		peer.Log().Error("Failed to register peer in sila syncer", "err", err)
		return err
	}
	if snap != nil {
	...
	for number, hash := range h.requiredBlocks {
		resCh := make(chan *sila.Response)

		req, err := peer.RequestHeadersByNumber(number, 1, 0, false, resCh)
		if err != nil {
			return err
		}
		go func(number uint64, hash common.Hash, req *sila.Request) {
			...
			select {
			case res := <-resCh:
				headers := ([]*types.Header)(*res.Res.(*sila.BlockHeadersRequest))
				if len(headers) == 0 {
...
// runSnapExtension registers a `snap` peer into the joint sila/snap peerset and
// starts handling inbound messages. As `snap` is only a satellite protocol to
// `sila`, all subsystem registrations and lifecycle management will be done by
// the main `sila` handler to prevent strange races.
...
func (h *handler) unregisterPeer(id string) {
	...
	// Remove the `sila` peer if it exists
...
func (h *handler) BroadcastTransactions(txs types.Transactions) {
	var (
		...
		txset = make(map[*silaPeer][]common.Hash) // Set peer->hash to transfer directly
		annos = make(map[*silaPeer][]common.Hash) // Set peer->hash to announce
	...
	for _, tx := range txs {
		var directSet map[*silaPeer]struct{}
		switch {
...
type blockRangeState struct {
	prev    sila.BlockRangeUpdatePacket
	next    atomic.Pointer[sila.BlockRangeUpdatePacket]
...
func (h *handler) blockRangeLoop(st *blockRangeState) {
	...
	for {
		select {
		case ev := <-st.syncCh:
			if ev.Type == downloader.SyncStarted && ev.Mode == silaconfig.SnapSync {
...
func (st *blockRangeState) update(chain *core.BlockChain, latest *types.Header) {
	...
	st.next.Store(&sila.BlockRangeUpdatePacket{
...
func (st *blockRangeState) currentRange() sila.BlockRangeUpdatePacket {
	return *st.next.Load()
}
...
func newBroadcastChoice(self enode.ID, key [16]byte) *broadcastChoice {
	return &broadcastChoice{
		self:   self,
		key:    key,
		buffer: make(map[*silaPeer]struct{}),
	}
}
...
func (bc *broadcastChoice) choosePeers(peers []*silaPeer, txSender common.Address) map[*silaPeer]struct{} {
