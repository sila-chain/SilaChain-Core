...
// V1Database is a persistent key-value store based on the pebble v1 storage silaEngine.
// Apart from basic data storage functionality it also supports batch writes and
// iterating over the keyspace in binary-alphabetical order.
type V1Database struct {
	fn        string     // filename for reporting
	db        *pebble.DB // Underlying pebble storage silaEngine
...
func NewV1(file string, cache int, handles int, namespace string, readonly bool) (*V1Database, error) {
	...
	opt := &pebble.Options{
		...
		WALBytesPerSync: 5 * siladb.IdealBatchSize,
...
func (d *V1Database) NewBatch() siladb.Batch {
	return &v1batch{
		b:  d.db.NewBatch(),
		db: d,
	}
...
func (d *V1Database) NewBatchWithSize(size int) siladb.Batch {
	return &v1batch{
...
func (d *V1Database) NewIterator(prefix []byte, start []byte) siladb.Iterator {
...
func (b *v1batch) Replay(w siladb.KeyValueWriter) error {
	reader := b.b.Reader()
	for {
		...
		if kind == pebble.InternalKeyKindSet {
			...
			if rangeDeleter, ok := w.(siladb.KeyValueRangeDeleter); ok {
				if err = rangeDeleter.DeleteRange(k, v); err != nil {
					return err
				}
			} else {
				return errors.New("siladb.KeyValueWriter does not implement DeleteRange")
			}
		} else {
...
// v1pebbleIterator is a wrapper of underlying iterator in storage silaEngine.
