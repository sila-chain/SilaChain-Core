...
// Database is a persistent key-value store based on the pebble v2 storage silaEngine.
...
func (d *Database) NewBatch() siladb.Batch {
	return &batch{
		b:  d.db.NewBatch(),
		db: d,
	}
...
func (d *Database) NewBatchWithSize(size int) siladb.Batch {
	return &batch{
...
func (b *batch) Replay(w siladb.KeyValueWriter) error {
	reader := b.b.Reader()
	for {
...
// pebbleIterator is a wrapper of underlying iterator in storage silaEngine.
...
func (d *Database) NewIterator(prefix []byte, start []byte) siladb.Iterator {
