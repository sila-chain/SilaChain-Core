...
func (db *Database) SyncKeyValue() error {
	...
	// Additionally, LevelDB is no longer the default database silaEngine and is likely
...
func (r *replayer) DeleteRange(start, end []byte) {
	...
	if rangeDeleter, ok := r.writer.(siladb.KeyValueRangeDeleter); ok {
		r.failure = rangeDeleter.DeleteRange(start, end)
	} else {
		r.failure = errors.New("siladb.KeyValueWriter does not implement DeleteRange")
	}
}
