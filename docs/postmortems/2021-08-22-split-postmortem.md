...
- 2021-08-24: Released [v1.10.8](https://github.com/sila-org/sila/releases/tag/v1.10.8) containing the fix on Tuesday morning (CET). Erigon released [v2021.08.04](https://github.com/ledgerwatch/erigon/releases/tag/v2021.08.04).
...
On 2021-08-17, Guido Vranken submitted a report to bounty@sila.org. This coincided with a sila-meetup in Berlin, so the sila team could fairly quickly analyse the issue. 
...
Since we had merged the removal of `SILA65`, if the entire network were to upgrade, then nodes which have not yet implemented `SILA66` would be cut off from the network. After further discussions, we decided to:
...
- Temporarily revert the `SILA65`-removal.
- Place the fix into the PR optimizing the jumpdest analysis [23381](https://github.com/sila-org/sila/pull/23381). 
- After 4-8 weeks, release details about the vulnerability. 


## Exploit

At block [13107518](https://etherscan.io/block/13107518), mined at Aug-27-2021 12:50:07 PM +UTC, a minority chain split occurred. The discord user @AlexSSD7 notified the allcoredevs-channel on the Sila R&D discord, on Aug 27 13:09  UTC. 
...
The sila-team have an official policy regarding [vulnerability disclosure](https://sila.org/docs/developers/sila-developer/disclosures). 
...
```
We have identified a security issue with sila, and will issue a
...
upgrade to the latest sila codebase. More information about the
