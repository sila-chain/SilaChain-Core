...
func benchmarkFilters(b *testing.B, history uint64, noHistory bool) {
	...
	defer db.Close()
	_, chain, receipts := core.GenerateChainWithGenesis(gspec, ethash.NewSilaPoWFaker(), 100010, func(i int, gen *core.BlockGen) {
		switch i {
		case 2403:
...
func testFilters(t *testing.T, history uint64, noHistory bool) {
	...
	pchain, preceipts := core.GenerateChain(gspec.Config, chain[len(chain)-1], ethash.NewSilaPoWFaker(), db, 1, func(i int, gen *core.BlockGen) {
...
func TestRangeLimit(t *testing.T) {
	...
	chain, _ := core.GenerateChain(gspec.Config, gspec.ToBlock(), ethash.NewSilaPoWFaker(), db, 10, func(i int, gen *core.BlockGen) {})
	options := core.DefaultConfig().WithStateScheme(rawdb.HashScheme)
