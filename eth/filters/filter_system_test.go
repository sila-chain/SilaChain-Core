...
func TestBlockSubscription(t *testing.T) {
	...
	var (
		...
		_, chain, _ = core.GenerateChainWithGenesis(genesis, ethash.NewSilaPoWFaker(), 10, func(i int, gen *core.BlockGen) {})
...
func TestInvalidGetLogsRequest(t *testing.T) {
	...
	var (
		...
		db, blocks, _    = core.GenerateChainWithGenesis(genesis, ethash.NewSilaPoWFaker(), 10, func(i int, gen *core.BlockGen) {})
	...
	blockchain, err := core.NewBlockChain(db, genesis, ethash.NewSilaPoWFaker(), nil)
	if err != nil {
...
func TestExceedLogQueryLimit(t *testing.T) {
	...
	chain, _ := core.GenerateChain(gspec.Config, gspec.ToBlock(), ethash.NewSilaPoWFaker(), db, 1000, func(i int, gen *core.BlockGen) {})

	options := core.DefaultConfig().WithStateScheme(rawdb.HashScheme)
	options.TxLookupLimit = 0 // index all txs
	bc, err := core.NewBlockChain(db, gspec, ethash.NewSilaPoWFaker(), options)
	if err != nil {
		t.Fatal(err)
	}
...
func TestTransactionReceiptsSubscription(t *testing.T) {
	...
	var (
		...
		_, chain, _ = core.GenerateChainWithGenesis(genesis, ethash.NewSilaPoWFaker(), 1, func(i int, gen *core.BlockGen) {
