...
func (args *BuildPayloadArgs) Id() silaEngine.PayloadID {
	hasher := sha256.New()
...
// silaEngine-api specification, EL should build the initial version of the payload
// which has an empty transaction set and then keep update it in order to maximize
// the revenue. Therefore, the empty-block here is always available and full-block
// will be set/updated afterwards.
type Payload struct {
	id            silaEngine.PayloadID
	empty         *types.Block
...
func newPayload(empty *types.Block, emptyRequests [][]byte, witness *stateless.Witness, id silaEngine.PayloadID) *Payload {
	payload := &Payload{
		id:            id,
		empty:         empty,
...
func (payload *Payload) Resolve() *silaEngine.ExecutionPayloadEnvelope {
	...
	if payload.full != nil {
		envelope := silaEngine.BlockToExecutableData(payload.full, payload.fullFees, payload.sidecars, payload.requests)
	...
	envelope := silaEngine.BlockToExecutableData(payload.empty, big.NewInt(0), nil, payload.emptyRequests)
...
func (miner *Miner) BuildTestingPayload(args *BuildPayloadArgs, transactions []*types.Transaction, empty bool, extraData []byte) (*types.Block, *silaEngine.ExecutionPayloadEnvelope, error) {
	...
	return res.block, silaEngine.BlockToExecutableData(res.block, res.fees, res.sidecars, res.requests), nil
}
