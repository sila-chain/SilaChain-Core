...
func (miner *Miner) generateWork(ctx context.Context, genParam *generateParams, witness bool) (result *newPayloadResult) {
	...
	miner.silaEngine.Finalize(miner.chain, work.header, work.state, &body, uint32(work.tcount+1), work.bal)
...
func (miner *Miner) prepareWork(ctx context.Context, genParams *generateParams, witness bool) (result *environment, err error) {
	...
	// Run the consensus preparation with the default or customized consensus silaEngine.
	// Note that the `header.Time` may be changed.
	if err := miner.silaEngine.Prepare(miner.chain, header); err != nil {
