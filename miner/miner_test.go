...
func createMiner(t *testing.T) *Miner {
	...
	// Create consensus silaEngine
	silaEngine := clique.New(chainConfig.Clique, chainDB)
	// Create Sila backend
	bc, err := core.NewBlockChain(chainDB, genesis, silaEngine, nil)
	if err != nil {
	...
	miner := New(backend, config, silaEngine)
	return miner
}
