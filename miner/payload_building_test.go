...
func init() {
	...
	silaashChainConfig = new(params.ChainConfig)
	*silaashChainConfig = *params.TestChainConfig
...
func newTestWorkerBackend(t *testing.T, chainConfig *params.ChainConfig, silaEngine consensus.SilaEngine, db siladb.Database, n int) *testWorkerBackend {
	var gspec = &core.Genesis{
		Config: chainConfig,
		Alloc:  types.GenesisAlloc{testBankAddress: {Balance: testBankFunds}},
	}
	switch e := silaEngine.(type) {
	case *clique.Clique:
		gspec.ExtraData = make([]byte, 32+common.AddressLength+crypto.SignatureLength)
		copy(gspec.ExtraData[32:32+common.AddressLength], testBankAddress.Bytes())
		e.Authorize(testBankAddress)
	case *silaash.Silaash:
	default:
		t.Fatalf("unexpected consensus silaEngine type: %T", silaEngine)
	}
	chain, err := core.NewBlockChain(db, gspec, silaEngine, &core.BlockChainConfig{ArchiveMode: true})
	if err != nil {
...
func newTestWorker(t *testing.T, chainConfig *params.ChainConfig, silaEngine consensus.SilaEngine, db siladb.Database, blocks int) (*Miner, *testWorkerBackend) {
	backend := newTestWorkerBackend(t, chainConfig, silaEngine, db, blocks)
	backend.txPool.Add(pendingTxs, true)
	w := New(backend, testConfig, silaEngine)
	return w, backend
}
...
func TestBuildPayload(t *testing.T) {
	...
	w, b := newTestWorker(t, params.TestChainConfig, silaash.NewFaker(), db, 0)
