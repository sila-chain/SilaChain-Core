...
// This file contains a miner stress test based on the SilaEngine API flow.
package main
...
func main() {
	...
	// SilaEngine API (through the simulated beacon driver).
...
func makeNode(genesis *core.Genesis) (*node.Node, *sila.Sila, *catalyst.SimulatedBeacon, error) {
	...
	silaBackend, err := sila.New(stack, &silaconfig.Config{
		Genesis:            genesis,
		...
		GPO:                silaconfig.Defaults.GPO,
		Miner:              silaconfig.Defaults.Miner,
	...
	driver, err := catalyst.NewSimulatedBeacon(0, common.Address{}, silaBackend)
	if err != nil {
		return nil, nil, nil, err
	}
	...
	return stack, silaBackend, driver, nil
}
