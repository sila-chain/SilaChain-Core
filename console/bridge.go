...
type bridge struct {
	client   *rpc.Client         // RPC client to execute Sila console requests through
...
func (b *bridge) SleepBlocks(call jsre.Call) (goja.Value, error) {
	...
	if err := b.client.Call(&lastNumber, "sila_blockNumber"); err != nil {
		return nil, err
	}
	for time.Now().Before(deadline) {
		var number hexutil.Uint64
		if err := b.client.Call(&number, "sila_blockNumber"); err != nil {
			return nil, err
		}
