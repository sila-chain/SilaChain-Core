...
var (
	_ = sila.ChainReader(&silaclient.Client{})
	_ = sila.TransactionReader(&silaclient.Client{})
	_ = sila.ChainStateReader(&silaclient.Client{})
	_ = sila.ChainSyncReader(&silaclient.Client{})
	_ = sila.ContractCaller(&silaclient.Client{})
	_ = sila.GasEstimator(&silaclient.Client{})
	_ = sila.GasPricer(&silaclient.Client{})
	_ = sila.LogFilterer(&silaclient.Client{})
	_ = sila.PendingStateReader(&silaclient.Client{})
	// _ = sila.PendingStateEventer(&silaclient.Client{})
	_ = sila.PendingContractCaller(&silaclient.Client{})
)
...
func generateTestChain() []*types.Block {
	...
	_, blocks, _ := core.GenerateChainWithGenesis(genesis, beacon.New(silaash.NewFaker()), 2, generate)
...
func testHeader(t *testing.T, chain []*types.Block, client *rpc.Client) {
	...
	for name, tt := range tests {
		t.Run(name, func(t *testing.T) {
			ec := silaclient.NewClient(client)
...
func sendTransaction(ec *silaclient.Client) error {
...
func ExampleRevertErrorData() {
	// First create an silaclient.Client instance.
	ctx := context.Background()
	ec, _ := silaclient.DialContext(ctx, exampleNode.HTTPEndpoint())
	...
	revertData, ok := silaclient.RevertErrorData(err)
	if !ok {
...
func TestSimulateV1(t *testing.T) {
	...
	client := silaclient.NewClient(backend.Attach())
	defer client.Close()
	...
	// Simple test: transfer SILA from one account to another
	from := testAddr
	...
	opts := silaclient.SimulateOptions{
		BlockStateCalls: []silaclient.SimulateBlock{
			{
				Calls: []sila.CallMsg{
					{
						From:      from,
...
func TestSimulateV1WithStateOverrides(t *testing.T) {
	...
	from := testAddr
	to := common.HexToAddress("0x0000000000000000000000000000000000000001")
	value := big.NewInt(1000000000000000000) // 1 SILA
	gas := uint64(100000)
	maxFeePerGas := new(big.Int).Mul(header.BaseFee, big.NewInt(2))
