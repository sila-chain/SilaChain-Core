...
//
// If you want to use the standardized Sila RPC functionality, use silaclient.Client instead.
type Client struct {
	c *rpc.Client
}
...
func (ec *Client) CreateAccessList(ctx context.Context, msg sila.CallMsg) (*types.AccessList, uint64, string, error) {
	...
	if err := ec.c.CallContext(ctx, &result, "sila_createAccessList", toCallArg(msg)); err != nil {
		return nil, 0, "", err
	}
...
func (ec *Client) GetProof(ctx context.Context, account common.Address, keys []string, blockNumber *big.Int) (*AccountResult, error) {
	...
	err := ec.c.CallContext(ctx, &res, "sila_getProof", account, keys, toBlockNumArg(blockNumber))
	if err != nil {
		return nil, err
	}
...
// the message call.
// Please use silaclient.CallContract instead if you don't need the override functionality.
...
func (ec *Client) SubscribeFullPendingTransactions(ctx context.Context, ch chan<- *types.Transaction) (*rpc.ClientSubscription, error) {
	return ec.c.SilaSubscribe(ctx, ch, "newPendingTransactions", true)
}

// SubscribePendingTransactions subscribes to new pending transaction hashes.
func (ec *Client) SubscribePendingTransactions(ctx context.Context, ch chan<- common.Hash) (*rpc.ClientSubscription, error) {
	return ec.c.SilaSubscribe(ctx, ch, "newPendingTransactions")
}
