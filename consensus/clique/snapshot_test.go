	...
	tests := []cliqueTest{
		{
			...
			// seems overly specific and weird, it was a Sila legacy testnet consensus split.
			epoch:   3,
			signers: []string{"A", "B", "C"},
			votes: []testerVote{
				{signer: "A"},
...
func (tt *cliqueTest) run(t *testing.T) {
	...
	silaEngine := New(config.Clique, rawdb.NewMemoryDatabase())
	silaEngine.fakeDiff = true

	_, blocks, _ := core.GenerateChainWithGenesis(genesis, silaEngine, len(tt.votes), func(j int, gen *core.BlockGen) {
	...
	chain, err := core.NewBlockChain(rawdb.NewMemoryDatabase(), genesis, silaEngine, nil)
	if err != nil {
	...
	snap, err := silaEngine.snapshot(chain, head.NumberU64(), head.Hash(), nil)
	if err != nil {
