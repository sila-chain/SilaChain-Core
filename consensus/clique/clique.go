...
// Package clique implements the proof-of-authority consensus silaEngine.
package clique
...
// New creates a Clique proof-of-authority consensus silaEngine with the initial
// signers set to the ones provided by the user.
func New(config *params.CliqueConfig, db siladb.Database) *Clique {
	// Set any missing consensus parameters to their defaults
	conf := *config
	if conf.Epoch == 0 {
		conf.Epoch = epochLength
	}
	// Allocate the snapshot caches and create the silaEngine
...
// Author implements consensus.SilaEngine, returning the Sila address recovered
...
// VerifyUncles implements consensus.SilaEngine, always returning an error for any
...
// Prepare implements consensus.SilaEngine, preparing all the consensus fields of the
...
// Finalize implements consensus.SilaEngine. There is no post-transaction
...
// Authorize injects a private key into the consensus silaEngine to mint new blocks
// with.
...
// Seal implements consensus.SilaEngine, attempting to create a sealed block using
...
// Close implements consensus.SilaEngine. It's a noop for clique as there are no background threads.
