...
func TestReimportMirroredState(t *testing.T) {
	...
	var (
		...
		silaEngine = New(params.AllCliqueProtocolChanges.Clique, db)
	...
	chain, _ := core.NewBlockChain(rawdb.NewMemoryDatabase(), genspec, silaEngine, nil)
	defer chain.Stop()

	_, blocks, _ := core.GenerateChainWithGenesis(genspec, silaEngine, 3, func(i int, block *core.BlockGen) {
	...
	db = rawdb.NewMemoryDatabase()
	chain, _ = core.NewBlockChain(db, genspec, silaEngine, nil)
	defer chain.Stop()
