...
// prevent silaEngine specific errors from being referenced in the remainder of the
// codebase, inherently breaking if the silaEngine is swapped out. Please put common
...
// Author implements consensus.SilaEngine, returning the header's coinbase as the
// proof-of-work verified author of the block.
func (silaash *Silaash) Author(header *types.Header) (common.Address, error) {
	return header.Coinbase, nil
}

// VerifyHeader checks whether a header conforms to the consensus rules of the
// stock Sila silaash silaEngine.
func (silaash *Silaash) VerifyHeader(chain consensus.ChainHeaderReader, header *types.Header) error {
	...
	return silaash.verifyHeader(chain, header, parent, false, time.Now().Unix())
}

// VerifyHeaders is similar to VerifyHeader, but verifies a batch of headers
// concurrently. The method returns a quit channel to abort the operations and
// a results channel to retrieve the async verifications.
func (silaash *Silaash) VerifyHeaders(chain consensus.ChainHeaderReader, headers []*types.Header) (chan<- struct{}, <-chan error) {
	// If we're running a full silaEngine faking, accept any input as valid
	if silaash.fakeFull || len(headers) == 0 {
	...
	go func() {
		for i, header := range headers {
			...
			if parent == nil {
				...
				err = silaash.verifyHeader(chain, header, parent, false, unixNow)
			}
			select {
			case <-abort:
				return
...
// rules of the stock Sila silaash silaEngine.
func (silaash *Silaash) VerifyUncles(chain consensus.ChainReader, block *types.Block) error {
	// If we're running a full silaEngine faking, accept any input as valid
	if silaash.fakeFull {
		return nil
	}
	...
	for _, uncle := range block.Uncles() {
		...
		if err := silaash.verifyHeader(chain, uncle, ancestors[uncle.ParentHash], true, time.Now().Unix()); err != nil {
			return err
		}
	}
	return nil
...
func (silaash *Silaash) verifyHeader(chain consensus.ChainHeaderReader, header, parent *types.Header, uncle bool, unixNow int64) error {
	...
	expected := silaash.CalcDifficulty(chain, header.Time, parent)
	...
	if chain.Config().IsShanghai(header.Number, header.Time) {
		return errors.New("silaash does not support shanghai fork")
	}
	...
	if chain.Config().IsCancun(header.Number, header.Time) {
		return errors.New("silaash does not support cancun fork")
	}
	...
	if silaash.fakeDelay != nil {
		time.Sleep(*silaash.fakeDelay)
	}
	if silaash.fakeFail != nil && *silaash.fakeFail == header.Number.Uint64() {
...
func (silaash *Silaash) CalcDifficulty(chain consensus.ChainHeaderReader, time uint64, parent *types.Header) *big.Int {
...
// Prepare implements consensus.SilaEngine, initializing the difficulty field of a
// header to conform to the silaash protocol. The changes are done inline.
func (silaash *Silaash) Prepare(chain consensus.ChainHeaderReader, header *types.Header) error {
	parent := chain.GetHeader(header.ParentHash, header.Number.Uint64()-1)
	if parent == nil {
		return consensus.ErrUnknownAncestor
	}
	header.Difficulty = silaash.CalcDifficulty(chain, header.Time, parent)
	return nil
}

// Finalize implements consensus.SilaEngine, accumulating the block and uncle rewards.
func (silaash *Silaash) Finalize(chain consensus.ChainHeaderReader, header *types.Header, state vm.StateDB, body *types.Body, blockAccessIndex uint32, bal *bal.ConstructionBlockAccessList) {
	// Accumulate any block and uncle rewards
	accumulateRewards(chain.Config(), state, header, body.Uncles)
}

// SealHash returns the hash of a block prior to it being sealed.
func (silaash *Silaash) SealHash(header *types.Header) (hash common.Hash) {
	...
	if header.WithdrawalsHash != nil {
		panic("withdrawal hash set on silaash")
	}
	if header.ExcessBlobGas != nil {
		panic("excess blob gas set on silaash")
	}
	if header.BlobGasUsed != nil {
		panic("blob gas used set on silaash")
	}
	if header.ParentBeaconRoot != nil {
		panic("parent beacon root set on silaash")
	}
	if header.SlotNumber != nil {
		panic("slot number set on silaash")
	}
