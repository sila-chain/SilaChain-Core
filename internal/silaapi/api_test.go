...
func newTestBackend(t *testing.T, n int, gspec *core.Genesis, silaEngine consensus.SilaEngine, generator func(i int, b *core.BlockGen)) *testBackend {
	options := core.DefaultConfig().WithArchive(true)
	options.TxLookupLimit = 0 // index all txs

	accman, acc := newTestAccountManager(t)
	gspec.Alloc[acc.Address] = types.Account{Balance: big.NewInt(params.SilaBaseUnit)}

	// Generate blocks for testing
	db, blocks, receipts := core.GenerateChainWithGenesis(gspec, silaEngine, n+1, generator)

	chain, err := core.NewBlockChain(db, gspec, silaEngine, options)
	if err != nil {
...
func TestEstimateGas(t *testing.T) {
	...
	var (
		...
		genesis  = &core.Genesis{
			...
			Alloc: types.GenesisAlloc{
				...
				accounts[2].addr: {Balance: big.NewInt(params.SilaBaseUnit), Code: append(types.DelegationPrefix, accounts[3].addr.Bytes()...)},
			},
		}
		genBlocks      = 10
	...
	api := NewBlockChainAPI(newTestBackend(t, genBlocks, genesis, beacon.New(silaash.NewFaker()), func(i int, b *core.BlockGen) {
	...
	var testSuite = []struct {
		...
		},
		{
			...
			call:        TransactionArgs{},
			overrides: override.StateOverride{
				randomAccounts[0].addr: override.OverrideAccount{Balance: newRPCBalance(new(big.Int).Mul(big.NewInt(1), big.NewInt(params.SilaBaseUnit)))},
			},
			expectErr: nil,
			want:      53000,
		},
	...
	var (
		...
		genesis  = &core.Genesis{
			...
			Alloc: types.GenesisAlloc{
				...
				dad: {
					Balance: big.NewInt(params.SilaBaseUnit),
					Nonce:   1,
...
func TestSimulateV1(t *testing.T) {
	...
	var (
		...
		genesis      = &core.Genesis{
			...
			Alloc: types.GenesisAlloc{
				...
				//     }
				// }
				cac: {Balance: big.NewInt(params.SilaBaseUnit), Code: common.Hex2Bytes("610dad80ff")},
				bab: {
	...
	api := NewBlockChainAPI(newTestBackend(t, genBlocks, genesis, silaash.NewFaker(), func(i int, b *core.BlockGen) {
...
func TestSimulateV1ChainLinkage(t *testing.T) {
	var (
		...
		gspec        = &core.Genesis{
			Config: params.MergedTestChainConfig,
			Alloc: types.GenesisAlloc{
				sender:       {Balance: big.NewInt(params.SilaBaseUnit)},
	...
	backend := newTestBackend(t, 1, gspec, beacon.New(silaash.NewFaker()), func(i int, b *core.BlockGen) {
...
func TestSimulateV1TxSender(t *testing.T) {
	var (
		...
		gspec     = &core.Genesis{
			...
			Alloc: types.GenesisAlloc{
				sender:  {Balance: big.NewInt(params.SilaBaseUnit)},
				sender2: {Balance: big.NewInt(params.SilaBaseUnit)},
				sender3: {Balance: big.NewInt(params.SilaBaseUnit)},
			},
		}
		ctx = context.Background()
	)
	backend := newTestBackend(t, 0, gspec, beacon.New(silaash.NewFaker()), func(i int, b *core.BlockGen) {})
	stateDB, baseHeader, err := backend.StateAndHeaderByNumberOrHash(ctx, rpc.BlockNumberOrHashWithNumber(rpc.LatestBlockNumber))
	if err != nil {
		t.Fatalf("failed to get state and header: %v", err)
	}
...
func TestSignTransaction(t *testing.T) {
	...
	b := newTestBackend(t, 1, genesis, beacon.New(silaash.NewFaker()), func(i int, b *core.BlockGen) {
		b.SetPoS()
	})
...
func TestRPCGetBlockOrHeader(t *testing.T) {
	...
	var (
		...
		genesis    = &core.Genesis{
			Config: params.TestChainConfig,
			Alloc: types.GenesisAlloc{
				acc1Addr: {Balance: big.NewInt(params.SilaBaseUnit)},
				acc2Addr: {Balance: big.NewInt(params.SilaBaseUnit)},
			},
		}
		genBlocks = 10
		signer    = types.HomesteadSigner{}
	)
	backend := newTestBackend(t, genBlocks, genesis, silaash.NewFaker(), func(i int, b *core.BlockGen) {
		// Transfer from account[0] to account[1]
		//    value: 1000 wei
		//    fee:   0 wei
...
func setupReceiptBackend(t *testing.T, genBlocks int) (*testBackend, []common.Hash) {
	...
	var (
		...
		genesis    = &core.Genesis{
			...
			Alloc: types.GenesisAlloc{
				...
				//     }
				// }
				contract: {Balance: big.NewInt(params.SilaBaseUnit), Code: common.FromHex("")},
			},
		}
	...
	backend := newTestBackend(t, genBlocks, genesis, beacon.New(silaash.NewFaker()), func(i int, b *core.BlockGen) {
		var (
...
func TestRPCGetTransactionReceipt(t *testing.T) {
	...
	for i, tt := range testSuite {
		...
		testRPCResponseWithFile(t, i, result, "sila_getTransactionReceipt", tt.file)
	}
}
...
func TestRPCGetBlockReceipts(t *testing.T) {
	...
	for i, tt := range testSuite {
		...
		testRPCResponseWithFile(t, i, result, "sila_getBlockReceipts", tt.file)
	}
}
...
func TestCreateAccessListWithStateOverrides(t *testing.T) {
	...
	backend := newTestBackend(t, 1, genesis, silaash.NewFaker(), nil)
...
func TestEstimateGasWithMovePrecompile(t *testing.T) {
	...
	backend := newTestBackend(t, 1, genesis, beacon.New(silaash.NewFaker()), func(i int, b *core.BlockGen) {
		b.SetPoS()
	})
	api := NewBlockChainAPI(backend)
...
func TestSIP7910Config(t *testing.T) {
	var (
		...
		// Define a snapshot of the current SilaDevTestnet config (only Prague scheduled) so that future forks do not
	...
	gspec := core.DefaultSilaDevTestnetGenesisBlock()
	gspec.Config = config
	...
	for i, tt := range testSuite {
		...
		testRPCResponseWithFile(t, i, result, "sila_config", tt.file)
	}
}
...
// makeSelfSignedRaw is a convenience for a 0-SILA self-transfer.
...
func TestSendRawTransactionSync_Success(t *testing.T) {
	...
	b := newTestBackend(t, 0, genesis, silaash.NewFaker(), nil)
...
func TestGetStorageValues(t *testing.T) {
	...
	api := NewBlockChainAPI(newTestBackend(t, 1, genesis, beacon.New(silaash.NewFaker()), func(i int, b *core.BlockGen) {
		b.SetPoS()
	}))
	latest := rpc.BlockNumberOrHashWithNumber(rpc.LatestBlockNumber)
...
func TestStateMethodsDefaultToLatest(t *testing.T) {
	...
	var (
		...
		genesis  = &core.Genesis{
			...
			Alloc: types.GenesisAlloc{
				accounts[0].addr: {Balance: big.NewInt(params.SilaBaseUnit)},
				accounts[1].addr: {
					Balance: big.NewInt(2 * params.SilaBaseUnit),
					Nonce:   7,
					Code:    code,
	...
	srv := rpc.NewServer()
	if err := srv.RegisterName("sila", NewBlockChainAPI(backend)); err != nil {
		t.Fatal(err)
	}
	if err := srv.RegisterName("sila", NewTransactionAPI(backend, new(AddrLocker))); err != nil {
		t.Fatal(err)
	}
