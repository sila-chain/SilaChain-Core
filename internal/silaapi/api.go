...
// proofList implements siladb.KeyValueWriter and collects the proofs as
...
func (context *ChainContext) SilaEngine() consensus.SilaEngine {
	return context.b.SilaEngine()
}
...
//
// https://sila.org/en/developers/docs/apis/json-rpc/#sila_sign
...
func checkTxFee(gasPrice *big.Int, gas uint64, cap float64) error {
	...
	feeSila := new(big.Float).Quo(new(big.Float).SetInt(new(big.Int).Mul(gasPrice, new(big.Int).SetUint64(gas))), new(big.Float).SetInt(big.NewInt(params.SilaBaseUnit)))
	feeFloat, _ := feeSila.Float64()
	if feeFloat > cap {
