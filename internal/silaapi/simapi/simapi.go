...
func SimulateV1(ctx context.Context, b Backend, opts SimOpts, blockNrOrHash *rpc.BlockNumberOrHash) ([]*SimBlockResult, error) {
	...
	state, base, err := b.StateAndHeaderByNumberOrHash(ctx, *blockNrOrHash)
	...
	sim := &simulator{
		...
		budget:         newGasBudget(b.RPCGasCap()),
...
type simCallResult = SimCallResult
type simBlockResult = SimBlockResult
type simOpts = SimOpts
...
func (sim *simulator) sanitizeCall(call *txargs.TransactionArgs, state vm.StateDB, header *types.Header, gp *core.GasPool) (bool, error) {
	if call.Nonce == nil {
