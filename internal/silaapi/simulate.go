...
// simOpts are the inputs to sila_simulateV1.
type simOpts struct {
...
func (sim *simulator) processBlock(ctx context.Context, block *simBlock, header, parent *types.Header, headers []*types.Header, timeout time.Duration) (*types.Block, []simCallResult, map[common.Hash]common.Address, error) {
	...
	sim.b.SilaEngine().Finalize(chainHeadReader, header, sim.state, blockBody, uint32(len(block.Calls)+1), blockAccessList)
...
func (sim *simulator) makeHeaders(blocks []simBlock) ([]*types.Header, error) {
	...
	for bi, block := range blocks {
		...
		// For example, calling sila_simulateV1(..., blockParameter: 0x0) on sila-dev-testnet network will cause all blocks to have a difficulty of 1 and be treated as pre-merge.
...
func (b *simBackend) SilaEngine() consensus.SilaEngine {
	return b.b.SilaEngine()
}
