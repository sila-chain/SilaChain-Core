...
func makeTestTrie(scheme string) (siladb.Database, *testDb, *StateTrie, map[string][]byte) {
...
// content map.
func checkTrieContents(t *testing.T, db siladb.Database, scheme string, root []byte, content map[string][]byte, rawTrie bool) {
...
func checkTrieConsistency(db siladb.Database, scheme string, root common.Hash, rawTrie bool) error {
	ndb := newTestDatabase(db, scheme)
	var it NodeIterator
	if rawTrie {
...
func syncWith(t *testing.T, root common.Hash, db siladb.Database, srcDb *testDb) {
	syncWithHookWriter(t, root, db, srcDb, nil)
}

func syncWithHookWriter(t *testing.T, root common.Hash, db siladb.Database, srcDb *testDb, hookWriter siladb.KeyValueWriter) {
	// Create a destination trie and sync with the scheduler
