# Security Policy

## Supported Versions

Please see [Releases](https://github.com/sila-org/sila/releases). We recommend using the [most recently released version](https://github.com/sila-org/sila/releases/latest).

## Audit reports

Audit reports are published in the `docs` folder: https://github.com/sila-org/sila/tree/master/docs/audits 

| Scope | Date | Report Link |
| ------- | ------- | ----------- |
| `sila` | 20170425 | [pdf](https://github.com/sila-org/sila/blob/master/docs/audits/2017-04-25_Sila-audit_Truesec.pdf) |
| `Discv5` | 20191015 | [pdf](https://github.com/sila-org/sila/blob/master/docs/audits/2019-10-15_Discv5_audit_LeastAuthority.pdf) |
| `Discv5` | 20200124 | [pdf](https://github.com/sila-org/sila/blob/master/docs/audits/2020-01-24_DiscV5_audit_Cure53.pdf) |

## Reporting a Vulnerability

**Please do not file a public ticket** mentioning the vulnerability.

To find out how to disclose a vulnerability in Sila visit [https://bounty.sila.org](https://bounty.sila.org) or email bounty@sila.org. Please read the [disclosure page](https://github.com/sila-org/sila/security/advisories?state=published) for more information about publicly disclosed security vulnerabilities.


The following key may be used to communicate sensitive information to developers.

Fingerprint: `AE96 ED96 9E47 9B00 84F3 E17F E88D 3334 FA5F 6A0A`

```
-----BEGIN PGP PUBLIC KEY BLOCK-----


=b5eA
-----END PGP PUBLIC KEY BLOCK-----
```
