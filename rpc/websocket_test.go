...
func TestClientWebsocketPing(t *testing.T) {
	...
	sub, err := client.SilaSubscribe(ctx, resultChan, "foo")
	if err != nil {
...
func wsPingTestHandler(t *testing.T, conn *websocket.Conn, shutdown, sendPing <-chan struct{}) {
	// Canned responses for the sila_subscribe call in TestClientWebsocketPing.
	const (
		subResp   = `{"jsonrpc":"2.0","id":1,"result":"0x00"}`
		subNotify = `{"jsonrpc":"2.0","method":"sila_subscription","params":{"subscription":"0x00","result":1}}`
	)
