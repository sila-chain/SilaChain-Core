...
// Package silastats implements the network stats reporting service.
package silastats
...
// backend encompasses the bare-minimum functionality needed for silastats reporting
...
func New(node *node.Node, backend backend, silaEngine consensus.SilaEngine, url string) error {
...
func (s *Service) login(conn *connWrapper) error {
	...
	var network string
	if info := infos.Protocols["sila"]; info != nil {
		network = fmt.Sprintf("%d", info.(*ethproto.NodeInfo).Network)
	} else {
		return errors.New("no sila protocol available")
	}
	auth := &authMsg{
		ID: s.node,
		Info: nodeInfo{
...
func (s *Service) reportLatency(conn *connWrapper) error {
	// Send the current time to the silastats server
	start := time.Now()
	...
	log.Trace("Sending measured latency to silastats", "latency", latency)
...
func (s *Service) reportSilaNewPayload(conn *connWrapper, ev core.SilaNewPayloadEvent) error {
	...
	log.Trace("Sending new payload to silastats", "number", details.Number, "hash", details.Hash)
...
func (s *Service) reportBlock(conn *connWrapper, header *types.Header) error {
	...
	log.Trace("Sending new block to silastats", "number", details.Number, "hash", details.Hash)
...
func (s *Service) assembleBlockStats(header *types.Header) *blockStats {
	...
	author, _ := s.silaEngine.Author(header)
...
func (s *Service) reportHistory(conn *connWrapper, list []uint64) error {
	...
	if len(history) > 0 {
		log.Trace("Sending historical blocks to silastats", "first", history[0].Number, "last", history[len(history)-1].Number)
	} else {
...
func (s *Service) reportPending(conn *connWrapper) error {
	...
	log.Trace("Sending pending transactions to silastats", "count", pending)
...
func (s *Service) reportStats(conn *connWrapper) error {
	...
	log.Trace("Sending node details to silastats")
