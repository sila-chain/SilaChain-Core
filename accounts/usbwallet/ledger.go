...
// the legacy Ledger wallet application protocol
...
const (
	ledgerOpRetrieveAddress  ledgerOpcode = 0x02 // Returns the public key and legacy-compatible address for a given BIP 32 path
	ledgerOpSignTransaction  ledgerOpcode = 0x04 // Signs a SilaChain transaction after having the user validate the parameters
	ledgerOpGetConfiguration ledgerOpcode = 0x06 // Returns specific wallet application configuration
	ledgerOpSignTypedMessage ledgerOpcode = 0x0c // Signs a typed message following the EIP 712 specification
...
func (w *ledgerDriver) Open(device io.ReadWriter, passphrase string) error {
	...
	_, err := w.ledgerDerive(accounts.SilaBaseDerivationPath)
	if err != nil {
		// Wallet app is not running or in browser mode, nothing more to do, return
		if err == errLedgerReplyInvalidHeader {
			w.browser = true
		}
		return nil
	}
	// Try to resolve the wallet app's version, will fail prior to v1.0.2
...
// and returning the legacy-compatible address located on that derivation path.
...
//
// Note, if the version of the wallet application running on the Ledger wallet is
...
// ledgerVersion retrieves the current version of the wallet app running
...
// ledgerDerive retrieves the currently active legacy-compatible address from a Ledger
